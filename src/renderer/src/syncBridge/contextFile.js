...
export const cancelContextWrite = () => {
    if (timer !== null) {
        clearTimeout(timer);
        timer = null;
    }
 ...
 */
export const makeContextFileHandler = () => (state, prev) => {
    const selectedId = state.selectedElementIds[0];
    const prevSelectedId = prev.selectedElementIds[0];
    const changed = state.activePage !== prev.activePage ||
...
export const flushContextWrite = () => {
    cancelContextWrite();
    write();
};
