 ...
 */
export const makeOnWriteConflict = (ctx) => (target, conflict, silent = false, intent) => {
    ...
    if (intent !== undefined) {
        ...
        if (merged) {
            ...
            store.reloadElements(merged.parsed.elements, { tsx: merged.tsx, css: merged.css }, merged.parsed.customMediaBlocks, merged.parsed.keyframesBlocks, merged.parsed.cssDuplicates);
            dispatchPageWrite({
                kind: 'write',
            ...
            if (!silent) {
                useAppLogStore
                    .getState()
                    .log('info', `${target.name} was edited outside Scamp; merged that edit with yours.`);
            }
            return;
        }
    }
...
export const makeWriteIfDirty = (ctx) => (elements, rootElementId, target, customMediaBlocks, pageKeyframesBlocks, silent = false) => {
    ...
    if (fellBack.length > 0) {
        useAppLogStore
            .getState()
            .log('warn', `${target.name}: couldn't patch the ${fellBack.join(' or ')} safely; wrote the whole file instead.`);
    }
    ...
    dispatchPageWrite({
        ...
        ...(expectedTsx !== null && expectedCss !== null
            ...
            : {}),
    }, (conflict) => ctx.onWriteConflict(target, conflict, silent, expectedTsx !== null && expectedCss !== null
        ? {
            baseTsx: expectedTsx,
            baseCss: expectedCss,
            oursTsx: tsxOut.tsx,
