...
export const snapshotInputFrom = (state) => {
    ...
    return {
        ...
        viewNames: Object.entries(state.componentTrees)
            ...
            .map(([name]) => name),
        trees: Object.fromEntries(Object.entries(state.componentTrees).map(([name, tree]) => [
            name,
 ...
 */
export const installMcpResponder = () => window.scamp.onMcpQuery(({ requestId, tool, args }) => {
    try {
