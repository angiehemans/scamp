 ...
 */
const isChromeNode = (node: Node): boolean => {
 ...
 */
const withTransformReset = async <T>(
  node: HTMLElement,
  fn: () => Promise<T>
): Promise<T> => {
 ...
 */
export const sanitizeExportFilename = (raw: string): string =>
  raw.replace(/[\\/:*?"<>|]+/g, '').trim() || 'export';
 ...
 */
export const suggestExportFilename = (name: string): string =>
