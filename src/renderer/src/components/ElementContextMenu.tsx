...
export const ElementContextMenu = (): JSX.Element | null => {
  ...
  const targetRepeat = useCanvasStore((s) =>
    menu ? s.elements[menu.elementId]?.repeat : undefined
  );
  const targetShowIf = useCanvasStore((s) =>
    menu ? s.elements[menu.elementId]?.showIf : undefined
  );
