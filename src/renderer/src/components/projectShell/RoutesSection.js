...
export const RoutesSection = ({ routes, views, busy, onOpen, onSetRender, onGenerate }) => {
    ...
    return (_jsxs("div", { className: shellStyles.sidebarSection, "data-testid": "routes-section", children: [_jsx("h2", { className: shellStyles.sidebarTitle, children: "Routes" }), routes.length === 0 && unrouted.length === 0 && (_jsx("p", { className: styles.empty, children: "No routes yet. Add a page to get a view, then generate its route." })), _jsxs("ul", { className: shellStyles.pageList, children: [routes.map((route) => (_jsxs("li", { className: styles.routeRow, "data-testid": `route-${route.file}`, children: [_jsx(Tooltip, { label: `routes/${route.file} — open in the code panel`, children: _jsxs("button", { type: "button", className: styles.routeButton, onClick: () => onOpen(route.file), children: [route.path, _jsx("span", { className: styles.routeKind, children: route.kind === 'api' ? 'api' : (route.view ?? '') })] }) }), route.kind === 'page' && (_jsx("div", { className: styles.render, role: "group", "aria-label": `Render mode for ${route.path}`, children: RENDER_MODES.map((mode) => (_jsx(Tooltip, { label: RENDER_HELP[mode], children: _jsx("button", { type: "button", className: styles.renderOption, "aria-pressed": route.render === mode, disabled: busy, onClick: () => {
