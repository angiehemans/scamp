...
export const useCanvasKeyboardShortcuts = (
  keyDeps: { current: KeyboardShortcutDeps },
  componentEditor: {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent): void => {
      ...
      if ((e.metaKey || e.ctrlKey) && (e.key === 'p' || e.key === 'P')) {
        ...
        e.preventDefault();
        if (keyDeps.current.canPreview) keyDeps.current.openPreview();
        return;
      }
      ...
      if ((e.metaKey || e.ctrlKey) && (e.key === 'v' || e.key === 'V') && !e.shiftKey) {
        ...
        void (async (): Promise<void> => {
          ...
          if ((!s.activePage && !s.activeComponent) || !s.elements[ROOT_ELEMENT_ID]) return;
