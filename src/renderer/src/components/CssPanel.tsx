...
const buildBreakpointBody = (el: ScampElement, breakpointId: string): string => {
...
/**
 * The (className, cssPath, media) tuple the editor was loaded against.
 * Tracked in a ref so the commit path always writes to the element
 * and breakpoint that was being edited — never to whatever happens
 * to be selected at blur time.
 */
type EditTarget = {
  className: string;
  cssPath: string;
  /** Undefined for a base-class patch, populated for a media-scoped one. */
...
export const CssPanel = (): JSX.Element => {
  ...
  const editorBody = useMemo(() => {
    if (!element) return '';
    if (isDesktop) return buildClassBody(element, parentElement);
  ...
  const lastBreakpointId = useRef<string>(activeBreakpointId);
  ...
  return (
    <>
      ...
      <div className={styles.hint}>{hintSuffix}</div>
    </>
  );
};
