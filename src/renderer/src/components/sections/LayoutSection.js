...
export const LayoutSection = ({ elementId }) => {
    ...
    return (_jsxs(Section, { title: "Layout", elementId: elementId, fields: [
            ...
            'row-gap',
            'justify-items',
        ], children: [_jsx(Row, { label: "", children: _jsx(SegmentedControl, { value: mode, options: LAYOUT_OPTIONS, onChange: (value) => patchElement(elementId, computeLayoutPatch(element, value)), title: "Display mode" }) }), isFlex && (_jsx(_Fragment, { children: _jsxs(Row, { label: "", children: [_jsx(AlignmentGrid, { direction: element.flexDirection, alignItems: element.alignItems, justifyContent: element.justifyContent, onChange: (patch) => patchElement(elementId, patch) }), _jsxs("div", { style: {
                                display: 'grid',
                                gap: 6,
                                alignContent: 'center',
                                minWidth: 0,
                            }, children: [_jsx(EnumSelect, { value: element.alignItems, options: ALIGN_OPTIONS, onChange: (value) => patchElement(elementId, { alignItems: value }), title: "Align items" }), _jsx(EnumSelect, { value: element.justifyContent, options: JUSTIFY_OPTIONS, onChange: (value) => patchElement(elementId, { justifyContent: value }), title: "Justify content" }), _jsx(SpaceValueInput, { prefix: _jsx(GapIcon, { orientation: element.flexDirection === 'column'
                                            ? 'vertical'
                                            : 'horizontal' }), label: "Gap", title: "Gap between flex children", value: element.gap, onChange: (value) => patchElement(elementId, { gap: value }), min: 0, tokens: spacingTokens, ...(openThemePanel ? { onOpenTheme: openThemePanel } : {}) })] })] }) })), isGrid && (_jsxs(_Fragment, { children: [_jsx(Row, { label: "", children: _jsx(GridTemplateEditor, { columns: element.gridTemplateColumns, rows: element.gridTemplateRows, onChange: (patch) => patchElement(elementId, patch) }) }), _jsxs(Row, { label: "", children: [_jsx(SpaceValueInput, { prefix: _jsx(GapIcon, { orientation: "horizontal" }), label: "Column gap", title: "column-gap", value: element.columnGap, onChange: (value) => patchElement(elementId, { columnGap: value }), min: 0, tokens: spacingTokens, ...(openThemePanel ? { onOpenTheme: openThemePanel } : {}) }), _jsx(SpaceValueInput, { prefix: _jsx(GapIcon, { orientation: "vertical" }), label: "Row gap", title: "row-gap", value: element.rowGap, onChange: (value) => patchElement(elementId, { rowGap: value }), min: 0, tokens: spacingTokens, ...(openThemePanel ? { onOpenTheme: openThemePanel } : {}) })] }), _jsxs(Row, { label: "", children: [_jsx(EnumSelect, { value: element.alignItems, options: ALIGN_OPTIONS, onChange: (value) => patchElement(elementId, { alignItems: value }), title: "Align items" }), _jsx(EnumSelect, { value: element.justifyItems, options: GRID_SELF_OPTIONS, onChange: (value) => patchElement(elementId, { justifyItems: value }), title: "Justify items" })] })] }))] }));
};
