 ...
 * to the right axis.
 */
export const AnimationSection = ({ elementId }) => {
    ...
    if (!animation && !multiAnimationRaw) {
        return (_jsx(Section, { title: "Animation", collapsible: true, defaultOpen: false, elementId: elementId, fields: ['animation'], cssProperties: ['animation'], children: _jsx(Row, { label: "Preset", tooltip: "Pick an animation to play on this element. Choose None to remove.", children: _jsx(PresetSelect, { value: NONE_VALUE, onChange: handleSelectPreset }) }) }));
    }
    // Multi-animation source → can't model, defer to CSS mode.
    if (!animation && multiAnimationRaw) {
        return (_jsx(Section, { title: "Animation", collapsible: true, defaultOpen: false, elementId: elementId, fields: ['animation'], cssProperties: ['animation'], children: _jsx("div", { className: styles.hint, children: "Multiple animations declared. Edit in CSS mode." }) }));
    }
    if (!animation)
        return null;
    const isLoop = animation.iterationCount === 'infinite' || animation.iterationCount > 1;
    const stateMode = activeStateName;
    const showHoverRestartHint = stateMode !== null && animation.iterationCount === 'infinite';
    ...
    return (_jsxs(Section, { title: "Animation", collapsible: true, defaultOpen: true, elementId: elementId, fields: ['animation'], cssProperties: ['animation'], children: [_jsx(Row, { label: "Preset", tooltip: "The named animation. Switching presets replaces all the properties below with that preset's defaults.", children: _jsx(PresetSelect, { value: animation.name, onChange: handleSelectPreset, isCustom: isCustom }) }), stateMode !== null && !stateHasOverride && (_jsxs("div", { className: styles.hint, children: ["Same as default \u2014 edits will create a ", stateMode, "-state override."] })), _jsx(DualField, { left: {
                    ...
                    label: 'Easing',
                    tooltip: 'The acceleration curve. ease starts and ends slowly; linear is constant; ease-in starts slow; ease-out ends slow; ease-in-out is slow at both ends.',
                    children: (_jsx(EnumSelect, { value: NAMED_EASINGS.includes(animation.easing)
                            ? animation.easing
                            : 'ease', options: NAMED_EASINGS.map((e) => ({ value: e, label: e })), onChange: (easing) => updateField({ easing }) })),
                } }), _jsx(DualField, { left: {
                    label: 'Delay',
                    tooltip: 'Wait this long after the element is rendered before the animation starts.',
                    children: (_jsx(NumberInput, { value: animation.delayMs, onChange: (ms) => ms !== undefined && updateField({ delayMs: ms }), min: 0, suffix: "ms" })),
                }, right: {
                    label: 'Iteration',
                    tooltip: 'How many times the animation repeats. Use the dropdown to switch to an infinite loop.',
                    children: (_jsx(IterationField, { value: animation.iterationCount, onChange: (next) => updateField({ iterationCount: next }) })),
                } }), _jsx(DualField, { left: {
                    label: 'Direction',
                    tooltip: 'Normal plays the animation forward each iteration.\n' +
                    ...
                    children: (_jsx(EnumSelect, { value: animation.direction, options: DIRECTION_OPTIONS, onChange: (direction) => updateField({ direction }) })),
                }, right: {
                    label: 'Fill mode',
                    tooltip: 'Which styles apply outside the animation playback.\n' +
...
const PresetSelect = ({ value, onChange, isCustom = false, }) => {
    ...
    return (_jsxs("select", { className: controlStyles.select, value: isCustom ? '__custom__' : value, onChange: (e) => onChange(e.target.value), children: [_jsx("option", { value: NONE_VALUE, children: "None" }), isCustom && (_jsxs("option", { value: "__custom__", disabled: true, children: ["Custom: ", value] })), Object.keys(grouped).map((category) => (_jsx("optgroup", { label: CATEGORY_LABEL[category], children: grouped[category].map((preset) => (_jsx("option", { value: preset.name, children: preset.name }, preset.name))) }, category)))] }));
};
/**
 ...
 * to 1.
 */
const IterationField = ({ value, onChange }) => {
    ...
    return (_jsxs("div", { className: styles.iterationWrapper, ref: wrapperRef, children: [isInfinite ? (_jsx(PrefixSuffixInput, { value: "Infinite", onCommit: () => undefined, disabled: true, suffix: caret })) : (_jsx(NumberInput, { value: value, onChange: (n) => n !== undefined && handleNumberCommit(n), min: 1, suffix: caret })), menuOpen && (_jsxs("div", { className: styles.iterationMenu, role: "menu", children: [_jsxs("button", { type: "button", role: "menuitemradio", "aria-checked": !isInfinite, className: `${styles.iterationMenuItem} ${!isInfinite ? styles.iterationMenuItemActive : ''}`, onClick: () => {
                            onChange(lastNumericRef.current);
                            setMenuOpen(false);
                        }, children: [_jsx("span", { className: styles.iterationMenuCheck, children: !isInfinite ? '✓' : '' }), "Number"] }), _jsxs("button", { type: "button", role: "menuitemradio", "aria-checked": isInfinite, className: `${styles.iterationMenuItem} ${isInfinite ? styles.iterationMenuItemActive : ''}`, onClick: () => {
                            onChange('infinite');
                            setMenuOpen(false);
                        }, children: [_jsx("span", { className: styles.iterationMenuCheck, children: isInfinite ? '✓' : '' }), "Infinite"] })] }))] }));
};
