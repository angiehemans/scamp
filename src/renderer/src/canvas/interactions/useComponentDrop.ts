 ...
 */
export const useComponentDrop = (geometry: CanvasGeometry): ComponentDrop => {
  const [drop, setDrop] = useState<ReparentDrop | null>(null);
  ...
  useEffect(() => {
    const handleDragEnd = (): void => setDrop(null);
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>): void => {
    ...
    const target = resolveComponentDrop(
      e.clientX,
      e.clientY,
      geometry,
      store.elements,
