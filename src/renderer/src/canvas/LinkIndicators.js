 ...
 */
export const LinkIndicators = ({ frameRef }) => {
    const elements = useCanvasStore((s) => s.elements);
    const pageNames = useCanvasStore((s) => s.pageNames);
    const selectedElementIds = useCanvasStore((s) => s.selectedElementIds);
    const requestPageNavigation = useCanvasStore((s) => s.requestPageNavigation);
    ...
    const measure = useCallback(() => {
        ...
        for (const { id, href } of linked) {
            ...
            next.push({
                ...
                pageName: classification.kind === 'page' || classification.kind === 'broken'
    ...
    return (_jsx(_Fragment, { children: indicators.map((ind) => {
            const tooltip = ind.broken
                ? `Links to /${ind.pageName ?? ''} (page not found in this project)`
                : ind.external
                    ? `Opens ${ind.href} in the system browser`
                    : `Links to ${ind.href}`;
            return (_jsx("button", { type: "button", className: `${styles.indicator} ${ind.broken ? styles.broken : styles.valid}`, style: {
                    left: ind.rect.x + ind.rect.w - INDICATOR_INSET,
                    top: ind.rect.y - INDICATOR_HALF,
                }, onPointerDown: (e) => e.stopPropagation(), onClick: (e) => handleClick(e, ind), title: tooltip, "aria-label": tooltip, children: _jsx(ChainIcon, { broken: ind.broken }) }, ind.elementId));
        }) }));
};
...
const ChainIcon = ({ broken }) => (_jsxs("svg", { width: "12", height: "12", viewBox: "0 0 16 16", fill: "none", xmlns: "http://www.w3.org/2000/svg", "aria-hidden": "true", children: [_jsx("path", { d: "M6.5 4.5h-1a3 3 0 0 0 0 6h1m3 0h1a3 3 0 0 0 0-6h-1m-3 3h4", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" }), broken && (_jsx("path", { d: "M2 14L14 2", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" }))] }));
