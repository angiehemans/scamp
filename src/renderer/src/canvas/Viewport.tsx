 ...
 */
const extentDebugEnabled = (): boolean => {
  try {
...
export const Viewport = ({
  ...
  useLayoutEffect(() => {
    ...
    const measure = (): void => {
      ...
      if (extentDebugEnabled()) {
        console.log(
          '[fit] containerWidth=%s fitWidth=%s (frameW=%s contentRight=%s clip=%s) -> scale=%s',
          w,
          fitWidth,
          frameW,
          content.right,
  ...
  const measureFrame = useCallback((): void => {
    ...
    if (extentDebugEnabled()) {
      console.log(
        '[extent] scale=%s frameClientWidth=%s rawRight=%s settled=%s widest=%s',
        appliedScale.toFixed(4),
        frame.clientWidth,
        right.toFixed(2),
        settleExtent(right, frame.clientWidth),
        widest || '(none — nothing past the frame)'
      );
    }
    ...
    setContent((prev) =>
      prev.right === nextRight && prev.bottom === nextBottom
        ? prev
