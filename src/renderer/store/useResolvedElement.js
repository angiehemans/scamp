 ...
 */
export const useResolvedElement = (elementId) => {
    ...
    if (!element)
        return undefined;
    return resolveElementAtState(element, activeBreakpointId, breakpoints, activeStateName);
};
/**
 ...
 */
export const useBreakpointOverrideFields = (elementId) => {
    const keys = useCanvasStore((s) => {
        if (s.activeBreakpointId === 'desktop')
            return EMPTY_KEYS;
        if (!elementId)
            return EMPTY_KEYS;
 ...
 */
export const useStateOverrideFields = (elementId) => {
    const keys = useCanvasStore((s) => {
        if (s.activeStateName === null)
            return EMPTY_STATE_KEYS;
        if (!elementId)
            return EMPTY_STATE_KEYS;
        const el = s.elements[elementId];
 ...
 */
export const useGroupToggle = (elementId, group, hasContent) => {
