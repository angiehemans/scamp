 ...
 */
const resolveElementName = (elements, id, fallback) => {
 ...
 */
export const formatHistoryLabel = (entry, elements) => {
 ...
 *   ≥ 24h   → absolute HH:MM
 */
export const formatRelativeTime = (ts, now) => {
    const ms = now - ts;
    if (ms < 30_000)
        return 'just now';
    if (ms < 60_000)
