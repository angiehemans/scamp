...
export function useListField<T extends object>(
  getList: () => ReadonlyArray<T>,
  setList: (next: ReadonlyArray<T>) => void
): ListField<T> {
  return {
    update: (index, patch): void =>
      setList(
        getList().map((item, i) => (i === index ? { ...item, ...patch } : item))
      ),
    remove: (index): void => setList(getList().filter((_, i) => i !== index)),
    add: (item): void => setList([...getList(), item]),
  };
}
