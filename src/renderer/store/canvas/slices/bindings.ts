...
import type { RepeatBinding, SampleRow, ScampElement } from '@lib/element';
...
export type BindingsSlice = Pick<
  CanvasState,
  | 'setAttributeBinding'
  | 'setEventBinding'
  | 'setRepeat'
...
const setOptional = <K extends keyof ScampElement>(
  el: ScampElement,
  key: K,
  value: ScampElement[K] | undefined
): ScampElement => {
  if (value === undefined) {
    const { [key]: _dropped, ...rest } = el;
    return rest as ScampElement;
  }
  return { ...el, [key]: value };
};

/** True when any element other than `except` still refers to `name` as a show flag or repeat list. */
const sampleStillUsed = (
...
export const createBindingsSlice: StateCreator<CanvasState, [], [], BindingsSlice> = (
  set
) => ({
  ...
  setEventBinding: (id, event, handler) => {
    ...
    set((state) => {
      const el = state.elements[id];
      if (!el) return state;
      if ((el.on?.[event] ?? null) === handler) return state;
      const on =
        handler === null ? withoutKey(el.on, event) : { ...(el.on ?? {}), [event]: handler };
      didChange = true;
  ...
  setRepeat: (id, repeat) => {
    ...
    set((state) => {
      ...
      if (!el || !root || id === state.rootElementId) return state;
      ...
      if (repeat === null) {
        ...
        if (!sampleStillUsed(elements, previous.over, id)) {
          elements[state.rootElementId] = setOptional(
            root,
            'samples',
            withoutKey(root.samples, previous.over)
          );
        }
        didChange = true;
        return { elements };
      }
      const next: RepeatBinding = repeat.key === undefined || repeat.key === 'id'
      ...
      if (!Array.isArray(samples[next.over])) samples[next.over] = [{ id: '1' }];
  ...
  setShowIf: (id, flag) => {
    ...
    set((state) => {
      ...
      const root = state.elements[state.rootElementId];
      if (!el || !root || id === state.rootElementId) return state;
      if ((el.showIf ?? null) === flag) return state;
      const elements = { ...state.elements };
      ...
      if (flag !== null && !isRowPath(flag)) {
        ...
        if (typeof samples[name] !== 'boolean') samples[name] = true;
      }
      ...
      elements[state.rootElementId] = setOptional(
        root,
        'samples',
        Object.keys(samples).length > 0 ? samples : undefined
      );
      didChange = true;
      return { elements };
    });
  ...
  setSampleFlag: (flag, value) => {
    ...
    set((state) => {
      const root = state.elements[state.rootElementId];
      if (!root || root.samples?.[flag] === value) return state;
      didChange = true;
  ...
  setSampleRows: (over, rows) => {
    ...
    set((state) => {
      ...
      const next: SampleRow[] = rows.map((row) => ({ ...row }));
      didChange = true;
      rootId = state.rootElementId;
      return {
        elements: {
  ...
  renameBindingProp: (oldName, newName) => {
    if (oldName === newName || newName.length === 0) return;
    const touched: string[] = [];
    set((state) => {
      const elements = { ...state.elements };
      const rename = (expr: string): string =>
        bindPropName(expr) === oldName ? `${expr.startsWith('!') ? '!' : ''}${newName}` : expr;
      for (const [id, el] of Object.entries(state.elements)) {
        let next = el;
        if (el.bind) {
          const bind = Object.fromEntries(Object.entries(el.bind).map(([k, v]) => [k, rename(v)]));
          if (JSON.stringify(bind) !== JSON.stringify(el.bind)) next = { ...next, bind };
        }
        if (el.on) {
          const on = Object.fromEntries(Object.entries(el.on).map(([k, v]) => [k, v === oldName ? newName : v]));
          if (JSON.stringify(on) !== JSON.stringify(el.on)) next = { ...next, on };
        }
        if (el.showIf !== undefined && bindPropName(el.showIf) === oldName) {
          next = { ...next, showIf: rename(el.showIf) };
        }
        if (el.repeat?.over === oldName) next = { ...next, repeat: { ...el.repeat, over: newName } };
