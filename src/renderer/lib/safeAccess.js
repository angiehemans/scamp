 ...
 */
export const requireGroup = (match, index) => {
    const value = match[index];
    if (value === undefined) {
        throw new Error(`Expected regex capture group ${index} in match "${match[0]}"`);
    }
    return value;
};
/**
 ...
 */
export const requireAt = (array, index) => {
    const value = array[index];
    if (value === undefined) {
        throw new Error(`Index ${index} out of bounds (length ${array.length})`);
    }
    return value;
};
