 ...
 */
const SHORTHAND_KEYWORD = /(^|\s)(no-repeat|repeat-x|repeat-y|space|round|fixed|local|scroll|border-box|padding-box|content-box)(\s|$)/i;
export const classifyBackgroundValue = (raw) => {
    const value = raw.trim();
    if (value.length === 0)
        return 'color';
    if (!IMAGE_FUNCTION.test(value))
        return 'color';
...
export const isImageBackground = (raw) => classifyBackgroundValue(raw) === 'image';
