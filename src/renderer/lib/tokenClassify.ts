...
const LENGTH_UNIT_RE =
  /^-?\d*\.?\d+(?:px|rem|em|%|pt|vw|vh|vmin|vmax|ch|ex)$/i;
...
export const classifyToken = (raw: string): TokenCategory => {
  ...
  //    rule.
  if (LENGTH_UNIT_RE.test(value)) return 'fontSize';

  // 2. Bare number → line-height. Covers `1.5`, `0`, `2`.
  if (UNITLESS_NUMBER_RE.test(value)) {
    const n = Number(value);
    if (Number.isFinite(n)) return 'lineHeight';
  }
  ...
  if (parts.some((p) => GENERIC_FAMILIES.has(p))) return 'fontFamily';

  // 4. Colour forms.
  if (HEX_RE.test(value)) return 'color';
  if (RGB_RE.test(value)) return 'color';
  if (HSL_RE.test(value)) return 'color';
  if (COMMON_NAMED_COLORS.has(lower)) return 'color';
