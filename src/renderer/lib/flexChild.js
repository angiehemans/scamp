 ...
 */
export const dropDrawnOffset = (element, parent) => {
    const placedByParent = parent !== undefined &&
        (parent.display === 'flex' || parent.display === 'grid' || parent.type === 'component-instance');
    if (!placedByParent || (element.x === 0 && element.y === 0))
        return element;
    return { ...element, x: 0, y: 0 };
};
export const preserveDrawnSize = (element, parent) => {
    if (parent?.display !== 'flex')
        return element;
    if (element.flexShrink !== DEFAULT_RECT_STYLES.flexShrink)
        return element;
 ...
 */
export const shrinkGuardPatch = (element, parent, axis, nextMode) => {
    if (parent?.display !== 'flex')
        return {};
    const mainAxis = isColumnDirection(parent.flexDirection)
        ? 'height'
        : 'width';
    if (axis !== mainAxis)
        return {};
