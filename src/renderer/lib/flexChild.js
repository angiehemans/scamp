...
export const preserveDrawnSize = (element, parent) => {
    if (parent?.display !== 'flex')
        return element;
    if (element.customProperties[FLEX_SHRINK] !== undefined)
        return element;
    return {
        ...element,
 ...
 */
export const releaseDrawnSize = (element, parent, patch) => {
    if (parent?.display !== 'flex')
        return patch;
    if (element.customProperties[FLEX_SHRINK] !== '0')
        return patch;
    const mainIsWidth = parent.flexDirection !== 'column';
    const nextMode = mainIsWidth ? patch.widthMode : patch.heightMode;
    if (nextMode === undefined || nextMode === 'fixed')
        return patch;
