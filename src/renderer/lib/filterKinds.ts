 ...
 */
export const FILTER_KINDS: ReadonlyArray<FilterKind> = [
  'blur',
  'brightness',
  'contrast',
  'grayscale',
...
const FILTER_KIND_SET: ReadonlySet<FilterKind> = new Set<FilterKind>(
  FILTER_KINDS
);

/** Type-guard for the parser. */
export const isFilterKind = (v: string): v is FilterKind =>
  (FILTER_KIND_SET as ReadonlySet<string>).has(v);
 ...
 */
export const FILTER_UNITS: Record<FilterKind, 'px' | '%' | 'deg'> = {
  blur: 'px',
  brightness: '%',
  contrast: '%',
  grayscale: '%',
 ...
 */
export const FILTER_DEFAULTS: Record<FilterKind, number> = {
  blur: 4,
  brightness: 100,
  contrast: 100,
  grayscale: 100,
 ...
 */
export const FILTER_RANGES: Record<FilterKind, { min: number; max: number }> = {
  blur: { min: 0, max: 100 },
