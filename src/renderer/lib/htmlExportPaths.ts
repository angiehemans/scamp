...
export const pageDepth = (pageName: string): number =>
  pageName === HOME_PAGE ? 0 : 1;

/** Path of a page's HTML file, relative to the export root. */
export const htmlPathFor = (pageName: string): string =>
  pageName === HOME_PAGE ? 'index.html' : `${pageName}/index.html`;

/** Path of a page's stylesheet, relative to the export root. */
export const cssPathFor = (pageName: string): string =>
  pageName === HOME_PAGE ? 'index.css' : `${pageName}/index.css`;
 ...
 * through this.
 */
export const rootRelativePrefix = (depth: number): string =>
  depth <= 0 ? '' : '../'.repeat(depth);

/** A page-relative reference to something at the export root. */
export const fromPageToRoot = (pageName: string, target: string): string =>
  `${rootRelativePrefix(pageDepth(pageName))}${target}`;
 ...
 * left alone.
 */
export const rewriteAssetUrlForPage = (
  url: string,
  pageName: string
): string => {
 ...
 */
export const rewriteHrefForPage = (
  href: string,
  fromPage: string,
  ...
  if (trimmed !== '/' && !/^\/[a-z0-9-]+$/i.test(trimmed)) return href;
  const classified = classifyHref(trimmed, pageNames);
  if (classified.kind !== 'page') return href;
