...
const isUsable = (n) => Number.isFinite(n) && n > 0;
/**
 ...
 */
export const thumbnailFrame = ({ sourceWidth, sourceHeight, aspect = THUMBNAIL_ASPECT, maxWidth = THUMBNAIL_MAX_WIDTH, }) => {
    if (!isUsable(sourceWidth) || !isUsable(sourceHeight))
        return null;
    if (!isUsable(aspect) || !isUsable(maxWidth))
        return null;
...
export const needsBackfill = (frame) => frame.dHeight < frame.outHeight;
