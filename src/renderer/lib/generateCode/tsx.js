...
const repeatKeyFor = (el) => el.repeat?.key ?? 'id';
/**
 ...
 */
const renderJsx = (el, elements, level, isComponent, repeatRow = null) => {
    const wrappers = (el.showIf !== undefined ? 1 : 0) + (el.repeat !== undefined ? 1 : 0);
    const row = el.repeat !== undefined ? { as: el.repeat.as, key: repeatKeyFor(el) } : repeatRow;
...
const renderElement = (el, elements, level, isComponent, repeatRow) => {
    ...
    if (el.type === 'image' && tag === 'img') {
        if (!('src' in bound))
            baseAttrs.push(formatTypedValue('src', el.src ?? ''));
        if (!('alt' in bound))
            baseAttrs.push(formatTypedValue('alt', el.alt ?? ''));
    }
...
const formatRow = (row) => {
    const fields = Object.entries(row).map(([field, value]) => typeof value === 'number' ? `${field}: ${value}` : `${field}: ${tsStringLiteral(value)}`);
    return `{ ${fields.join(', ')} }`;
};
/** The destructure entry for one prop: `name = default`, or a bare name. */
const formatDestructureEntry = (prop, rowIndent) => {
    const value = prop.defaultValue;
    if (value === undefined)
        return prop.name;
    if (typeof value === 'boolean')
        return `${prop.name} = ${value ? 'true' : 'false'}`;
    if (typeof value === 'string')
        return `${prop.name} = ${tsStringLiteral(value)}`;
    if (value.length === 0)
        return `${prop.name} = []`;
