 ...
 */
export const normalizeCopySelection = (
  ...
  for (const id of ids) {
    if (id === rootId) {
      ...
      if (root) for (const childId of root.childIds) expanded.add(childId);
      continue;
    }
    if (elements[id]) expanded.add(id);
  }

  // Drop anything already covered by an ancestor in the set.
  const roots = new Set<string>();
  for (const id of expanded) {
    if (ancestorIds(elements, id).some((a) => expanded.has(a))) continue;
    roots.add(id);
  }
  ...
  for (const id of roots) if (!seen.has(id)) ordered.push(id);
