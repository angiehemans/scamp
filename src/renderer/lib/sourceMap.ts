...
export const identityMap = (): SourceMap => ({
 ...
 * collect their edits.
 */
export const sourceMapOf = (replaced: ReadonlyArray<Replaced>): SourceMap => {
  if (replaced.length === 0) return identityMap();
  const at = (offset: number, edge: 'start' | 'end'): number => {
    let delta = 0;
    for (const region of replaced) {
      if (offset < region.start) break;
...
export type Rewrite = { start: number; end: number; text: string };
...
export const applyRewrites = (
  ...
  if (rewrites.length === 0) return { text: source, map: identityMap() };
  const ordered = [...rewrites].sort((a, b) => a.start - b.start);
  const replaced: Replaced[] = [];
  let out = '';
  let cursor = 0;
  for (const rewrite of ordered) {
    if (rewrite.start < cursor) continue; // overlapping; the first wins
 ...
 */
export const composeMaps = (first: SourceMap, second: SourceMap): SourceMap => ({
