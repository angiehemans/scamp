 ...
 */
export const effectiveDisplay = (
  el: Pick<ScampElement, 'display' | 'customProperties'> | null | undefined
): ScampElement['display'] | undefined => {
  if (!el) return undefined;
  const custom = el.customProperties['display'];
  if (custom === 'inline-flex') return 'flex';
  if (custom === 'inline-grid') return 'grid';
  return el.display;
};
