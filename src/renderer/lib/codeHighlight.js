 ...
 */
export const findTsxLines = (tsx, className) => {
    if (className.length === 0 || tsx.length === 0)
        return [];
    const attr = new RegExp(`data-scamp(?:-instance)?-id\\s*=\\s*"${escapeForRegex(className)}"`);
    const ranges = [];
    const lines = tsx.split('\n');
    for (let i = 0; i < lines.length; i += 1) {
        if (attr.test(lines[i] ?? ''))
            ranges.push({ from: i + 1, to: i + 1 });
    }
    return ranges;
};
/**
 ...
 */
const selectorTargets = (selector, className) => new RegExp(`\\.${escapeForRegex(className)}(?![\\w-])`).test(selector);
/** Strip comments and quoted strings so their braces can't be miscounted. */
const maskNonCode = (css) => {
    let out = '';
    let quote = null;
    let inComment = false;
 ...
 */
export const findCssBlocks = (css, className) => {
    if (className.length === 0 || css.length === 0)
        return [];
...
export const firstLine = (ranges) => ranges.length === 0 ? null : (ranges[0]?.from ?? null);
