 ...
 */
const TYPED_IMG_ATTRS: ReadonlySet<string> = new Set(['src', 'alt']);
 ...
 */
export const hasTypedSrcAlt = (el: ScampElement): boolean =>
  el.type === 'image' && (el.tag ?? 'img') === 'img';
...
export const attributeSample = (
  ...
  hasTypedSrcAlt(el) && TYPED_IMG_ATTRS.has(attr)
    ? attr === 'src'
      ? el.src
      : el.alt
    : el.attributes?.[attr];
...
export const withAttributeSample = (
  el: ScampElement,
  attr: string,
  value: string
): ScampElement =>
