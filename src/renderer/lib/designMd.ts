...
import { buildTextStyles, type TextStyleProp } from './typographyModel';
...
export const DESIGN_MD_SECTIONS: ReadonlyArray<string> = [
  'Overview',
  'Colors',
  'Typography',
  'Layout',
...
const colorEntries = (tokens: ReadonlyArray<ThemeToken>): [string, string][] =>
  tokens
...
const roleEntries = (
  ...
  tokensForRole(role, tokens).map((t) => [t.label, t.value]);

/** Text styles → typography objects with resolved concrete values. */
const typographyEntries = (
  tokens: ReadonlyArray<ThemeToken>
): [string, [string, string][]][] => {
  const keyFor: Record<TextStyleProp, string> = {
    family: 'fontFamily',
    size: 'fontSize',
    weight: 'fontWeight',
    leading: 'lineHeight',
  ...
  const resolve = (v: string): string => resolveTokenChain(v, tokens) ?? v;
  return buildTextStyles(tokens).map((style) => {
    const props: [string, string][] = [];
    (['family', 'size', 'weight', 'leading', 'tracking'] as TextStyleProp[])
      .forEach((p) => {
        const raw = style[p];
        if (raw !== null) props.push([keyFor[p], resolve(raw)]);
      });
...
const emitMap = (
  ...
  lines.push(`${key}:`);
  for (const [k, v] of entries) lines.push(`  ${yamlScalar(k)}: ${yamlScalar(v)}`);
};
...
export const generateDesignMd = (
  ...
  if (typo.length > 0) {
    ...
    for (const [name, props] of typo) {
      ...
      for (const [k, v] of props) yaml.push(`    ${k}: ${yamlScalar(v)}`);
    }
  }
  ...
  for (const section of DESIGN_MD_SECTIONS) {
    ...
    if (body) parts.push(`## ${section}`, '', body, '');
  }
  return parts.join('\n').replace(/\n+$/, '\n');
};
 ...
 */
export const parseDesignMd = (content: string): DesignProse => {
  ...
  const topLevel = (field: string): string | undefined => {
  ...
  const heads: { title: string; bodyStart: number; headStart: number }[] = [];
