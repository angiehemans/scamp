 ...
 */
const normaliseStopSelector = (raw: string): string => {
  const parts = raw
    .toLowerCase()
    .split(',')
    .map((s) => s.trim())
 ...
 */
const normaliseKeyframesBody = (body: string): string | null => {
 ...
 */
export const matchesPreset = (name: string, body: string): boolean => {
  if (!isPresetName(name)) return false;
  ...
  if (normalisedSource === null || normalisedCanonical === null) return false;
