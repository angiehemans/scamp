...
const selectionSection = (elements, el, extraSelected) => {
    const lines = [
        `ID:      ${el.id}`,
    ...
    if (parent)
        lines.push(`Parent:  .${classNameFor(parent)}`);
...
const stylesSection = (elements, el) => {
...
const childrenSection = (elements, el) => {
 ...
 */
export const buildContextMarkdown = (input) => {
    const { target, elements, selectedIds, canvasWidth, breakpointLabel } = input;
    const blocks = [HEADER, targetSection(target)];
    ...
    if (selected === undefined) {
        ...
        blocks.push(selectionSection(elements, selected, Math.max(0, selectedIds.length - 1)));
