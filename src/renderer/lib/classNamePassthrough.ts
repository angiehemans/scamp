 ...
 */
export const rootClassNameAttribute = (cssClass: string): string =>
  `className={\`\${styles.${cssClass}} \${className ?? ''}\`}`;

/** The prop name components accept and forward. */
export const PASSTHROUGH_PROP = 'className';

const PASSTHROUGH_RE =
  /className=\{`\$\{styles\.([A-Za-z_][A-Za-z0-9_]*)\}\s+\$\{\s*className\s*\?\?\s*(?:''|"")\s*\}`\}/g;
