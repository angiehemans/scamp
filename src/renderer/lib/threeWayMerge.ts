 ...
 */
import { applyEdits, diffText, type TextEdit } from './textEdits';
...
const sameEdit = (a: TextEdit, b: TextEdit): boolean =>
 ...
 */
const collides = (a: TextEdit, b: TextEdit): boolean => {
 ...
 */
export const mergeText = (base: string, ours: string, theirs: string): MergeResult => {
  if (ours === theirs) return { ok: true, text: ours, fromOurs: 0, fromTheirs: 0 };
  if (base === theirs) return { ok: true, text: ours, fromOurs: 0, fromTheirs: 0 };
  if (base === ours) return { ok: true, text: theirs, fromOurs: 0, fromTheirs: 0 };

  const oursEdits = diffText(base, ours);
  const theirsEdits = diffText(base, theirs);

  const overlaps: Array<{ start: number; end: number }> = [];
  const shared: TextEdit[] = [];
  ...
  if (overlaps.length > 0) return { ok: false, overlaps };
