...
export type GridTrack =
  ...
  | { kind: 'min-content' }
  | { kind: 'max-content' }
  // minmax() / fit-content() / var() / calc() / anything else that is a
  // single self-contained token — kept verbatim, still add/removable.
...
const topLevelComma = (raw: string): number => {
  let depth = 0;
...
const tokenToTrack = (raw: string): GridTrack => {
  const t = raw.trim();
  const lower = t.toLowerCase();
  if (lower === 'auto') return { kind: 'auto' };
  if (lower === 'min-content') return { kind: 'min-content' };
  if (lower === 'max-content') return { kind: 'max-content' };
 ...
 */
const expandRepeat = (token: string): GridTrack[] | null => {
...
export const parseGridTemplate = (template: string): GridTrack[] | null => {
  ...
  if (trimmed.includes('[')) return null; // named grid lines
  if (/\b(subgrid|masonry)\b/i.test(trimmed)) return null;
  ...
  if (tokens.some((t) => /^repeat\(/i.test(t))) return null;

  return tokens.map(tokenToTrack);
};

const trackToString = (t: GridTrack): string => {
  switch (t.kind) {
    case 'fr':
      return `${t.value}fr`;
    case 'px':
...
export const serializeGridTemplate = (tracks: ReadonlyArray<GridTrack>): string =>
  tracks.map(trackToString).join(' ');

/** N equal fractional tracks — the quick N×M picker's output. */
export const makeFrTracks = (n: number): GridTrack[] =>
  Array.from({ length: Math.max(0, n) }, () => ({ kind: 'fr', value: 1 }));
