...
const LENGTH_RE = /^(?:-?(?:\d+|\d*\.\d+)(?:px|%|r?em|vh|vw|vmin|vmax|ch)|0|var\(--[A-Za-z0-9_-]+\))$/;
...
const angle = (raw) => {
    ...
    if (m[2] === undefined && Number(requireGroup(m, 1)) !== 0)
        return null;
 ...
 */
export const parseTransformFunction = (segment) => {
 ...
 */
export const parseTransformList = (raw) => {
 ...
 */
export const formatTransformList = (transforms) => transforms.map(formatOne).join(' ');
