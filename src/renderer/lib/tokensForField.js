...
const typeEligible = (name) => !isTextStyleToken(name) && !isDesignRoleToken(name);
...
const lengthEligible = (name) => !isTextStyleToken(name);
const isLength = (t) => classifyToken(t.value) === 'fontSize';
/**
 ...
 */
export const tokensForField = (field, tokens) => {
    switch (field) {
        case 'fontFamily':
            return tokens.filter((t) => classifyToken(t.value) === 'fontFamily' && typeEligible(t.name));
        case 'lineHeight':
            return tokens.filter((t) => classifyToken(t.value) === 'lineHeight' && typeEligible(t.name));
        case 'fontSize':
            return prioritizeByPrefix(tokens.filter((t) => isLength(t) && typeEligible(t.name)), '--text-');
        case 'spacing':
            return prioritizeByPrefix(tokens.filter((t) => isLength(t) && lengthEligible(t.name)), '--space-');
        case 'letterSpacing':
            return prioritizeByPrefix(tokens.filter((t) => isLength(t) && lengthEligible(t.name)), '--space-');
        case 'borderWidth':
            return prioritizeByPrefix(tokens.filter((t) => isLength(t) && lengthEligible(t.name)), '--border-');
        case 'radius':
            return prioritizeByPrefix(tokens.filter((t) => isLength(t) && lengthEligible(t.name)), '--radius-');
        case 'shadow':
