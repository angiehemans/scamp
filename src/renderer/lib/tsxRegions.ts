...
const regionKey = (region: Pick<TsxRegion, 'kind' | 'key'>): string =>
  `${region.kind}:${region.key}`;
 ...
 * the parser.
 */
export const findTsxRegions = (source: string): TsxRegion[] => {
  ...
  const startOf = (i: number): number => offsets[i] ?? at;
  const endOf = (i: number): number => (i + 1 < offsets.length ? (offsets[i + 1] ?? at) : at);

  const regions: TsxRegion[] = [];
  const push = (kind: TsxRegionKind, key: string, from: number, to: number): void => {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (SCAMP_META.test(line)) push('scampMeta', '', i, i);
  }
  return regions;
};
 ...
 */
export const tsxRegionChanges = (base: string, next: string): TextEdit[] => {
