...
export const resetDelayFor = (status: HtmlExportStatus): number | null => {
  if (status === 'done') return DONE_VISIBLE_MS;
  if (status === 'error') return ERROR_VISIBLE_MS;
  return null;
};

/** Button text for each state. `detail` is the success summary, if any. */
export const exportButtonLabel = (
 ...
 */
export const exportButtonTooltip = (
  status: HtmlExportStatus,
  message?: string | null,
  location?: string | null
): string => {
  if (status === 'exporting') return 'Writing HTML and CSS files…';
  if (status === 'error') {
  ...
  if (status === 'done') {
    if (location && location.length > 0) return `Exported to ${location}`;
    return message && message.length > 0
      ? message
...
export const exportSummary = (fileCount: number, assetCount: number): string => {
  const files = `${fileCount} file${fileCount === 1 ? '' : 's'}`;
  if (assetCount <= 0) return `Exported ${files}`;
