 ...
 */
export const fixPathFromLoginShell = () => {
    if (process.platform === 'win32')
        return;
    const shell = process.env['SHELL'] || '/bin/zsh';
    try {
        const out = execFileSync(shell, ['-ilc', `printf '${MARKER}%s${MARKER}' "$PATH"`], { encoding: 'utf8', timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] });
        const resolved = out.split(MARKER)[1];
        if (!resolved)
            return;
        const merged = new Set();
        for (const p of resolved.split(delimiter))
            if (p)
                merged.add(p);
