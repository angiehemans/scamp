...
const createWindow = (): void => {
  ...
  setDevServerLogSink((payload) => {
    if (!win.isDestroyed()) win.webContents.send(IPC.DevServerLog, payload);
  });
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(IPC.AppReinitSentry, async (_e, optedIn: boolean) => {
 ...
 */
const snapshotOnShutdown = async (): Promise<void> => {
