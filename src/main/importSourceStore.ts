...
const rootFor = (projectPath: string): string =>
  join(
    tmpdir(),
    'scamp-import-source',
...
const metaPath = (dir: string): string => join(dir, 'source.json');
 ...
 */
export const setActiveImportProject = async (projectPath: string | null): Promise<void> => {
...
export const saveImportSource = async (
  projectPath: string,
  view: string,
  url: string,
...
export const listImportSources = async (projectPath: string): Promise<StoredSource[]> => {
  ...
  const out: Array<{ stored: StoredSource; at: number }> = [];
  for (const name of names) {
    const file = metaPath(join(root, name));
    try {
      const [raw, stat] = await Promise.all([fs.readFile(file, 'utf-8'), fs.stat(file)]);
      out.push({ stored: JSON.parse(raw) as StoredSource, at: stat.mtimeMs });
    } catch {
...
export const readImportSource = async (
  projectPath: string,
  view: string,
  part: 'html' | 'css',
  limit: number
): Promise<{ stored: StoredSource; text: string; truncated: boolean } | null> => {
  const dir = join(rootFor(projectPath), safeName(view));
  let stored: StoredSource;
  try {
    stored = JSON.parse(await fs.readFile(metaPath(dir), 'utf-8')) as StoredSource;
  } catch {
    return null;
  }
...
export const disposeImportSources = async (projectPath?: string): Promise<void> => {
  ...
  if (projectPath === undefined) activeProject = null;
};
