...
import type { UpdaterInfoPayload, UpdaterProgressPayload } from '@shared/types';
...
export const initAutoUpdater = (mainWindow: BrowserWindow): void => {
  if (!app.isPackaged || registered) return;
  registered = true;
  ...
  autoUpdater.on('update-available', (info) => {
    const payload: UpdaterInfoPayload = { version: info.version };
  ...
  autoUpdater.on('download-progress', (progress) => {
    const payload: UpdaterProgressPayload = {
