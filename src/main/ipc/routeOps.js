...
export const routePathFor = (file) => {
    if (!ROUTE_EXTENSION.test(file))
        return null;
    return segmentsToPath(file.replace(ROUTE_EXTENSION, '').split('/'));
};
export const isApiRouteFile = (file) => file === 'api' || file.startsWith('api/');
...
export const parseRenderExport = (tsx) => {
 ...
 */
export const setRenderExport = (tsx, mode) => {
    const line = `export const render = '${mode}';`;
    if (RENDER_RE.test(tsx))
        return tsx.replace(RENDER_RE, line);
    ...
    let end = last;
    while (end < lines.length - 1 && !/;\s*$/.test(lines[end] ?? '') && !/^\s*$/.test(lines[end + 1] ?? ''))
        end += 1;
...
export const listRoutes = async (projectPath) => {
...
const routeFilePath = (projectPath, file) => {
...
export const readRouteFile = (projectPath, file) => fs.readFile(routeFilePath(projectPath, file), 'utf-8');
export const setRouteRender = async (projectPath, file, render) => {
...
export const writeRouteFile = async (projectPath, file, content) => {
    const path = routeFilePath(projectPath, file);
    try {
...
export const readDevVarsKeys = async (projectPath) => {
    let text;
    try {
