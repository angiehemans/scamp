 ...
 */
const compareForDisplay = (
  a: StartScreenProject,
  b: StartScreenProject
): number => {
 ...
 * in via `recents`.
 */
export const mergeProjectsForDisplay = (
  recents: ReadonlyArray<RecentProject & { exists: boolean }>,
  scanned: ReadonlyArray<ScannedProject>
): StartScreenProject[] => {
  const byPath = new Map<string, StartScreenProject>();
 ...
 */
export const attachCardMeta = async (
