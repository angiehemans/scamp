 ...
 */
const isScampFormat = async (folderPath: string): Promise<boolean> => {
  try {
...
export const detectProjectFormat = async (
  ...
  if (await isScampFormat(folderPath)) return 'scamp';
  try {
  ...
  try {
    ...
    if (entries.some((e) => e.endsWith('.tsx'))) return 'legacy';
  } catch {
