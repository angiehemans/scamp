 ...
 * APFS and Linux.
 */
export const isScampProjectDir = async (
  folderPath: string
): Promise<boolean> => {
  try {
  ...
  try {
    ...
    if (entries.some((e) => e.endsWith('.tsx'))) return true;
  } catch {
 ...
 */
export const scanProjectsInFolder = async (
  folder: string
  ...
  const results = await Promise.all(
    dirs.map(async (dir): Promise<ScannedProject | null> => {
      const path = join(folder, dir.name);
      if (!(await isScampProjectDir(path))) return null;
      const format = await detectProjectFormat(path);
      return { name: dir.name, path, format };
    })
  );

  return results.filter((p): p is ScannedProject => p !== null);
};
