...
export const createPage = async (
  ...
  if (!PAGE_NAME_RE.test(args.pageName)) {
    throw new Error(
      `Invalid page name "${args.pageName}". Use alphanumeric and hyphens only.`
    );
  }
  if (format === 'nextjs' && args.pageName === 'home') {
    throw new Error(`A page named "home" already exists.`);
  }
  const { tsxPath, cssPath, pageDir } = pagePathsFor(
    args.projectPath,
    args.pageName,
    format
  );
...
export const deletePage = async (
  ...
  if (format === 'nextjs' && args.pageName === 'home') {
    throw new Error(
      `The "home" page can't be deleted in a Next.js project.`
    );
  }
  const { tsxPath, cssPath, pageDir } = pagePathsFor(
    args.projectPath,
