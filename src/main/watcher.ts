 ...
 */
const maybeNotifyRoutesChanged = (changedPath: string): void => {
  if (!mainWindow || mainWindow.isDestroyed() || !watchedPath) return;
  const rel = relative(watchedPath, changedPath);
  if (rel !== 'routes' && !rel.startsWith(`routes${sep}`)) return;
