...
export const __resetHeartbeatState = () => {
    lastSentAt = null;
};
export const sendHeartbeat = async (deps) => {
    ...
    let response;
    try {
        response = await deps.fetchImpl(new URL('/api/desktop/heartbeat', deps.baseUrl).toString(), {
            method: 'POST',
            headers: {
 ...
 */
export const startHeartbeat = (deps, intervalMs = HEARTBEAT_INTERVAL_MS) => {
