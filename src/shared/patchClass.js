 ...
 */
export const patchClassBlock = (source, className, newDeclarations, media) => {
 ...
 */
const patchInsideMedia = (root, className, declarations, media) => {
    ...
    if (!atRule) {
        ...
        const separator = root.toString().length === 0 || root.toString().endsWith('\n')
            ? ''
            : '\n';
    ...
    const classRule = postcss.parse(`.${className} {${declarations}}`).first;
    if (classRule) {
...
const buildMediaBlock = (media, className, declarations) => {
...
const appendMediaBlock = (source, media, className, declarations) => {
    const sep = source.length === 0 || source.endsWith('\n') ? '\n' : '\n\n';
    return `${source}${sep}${buildMediaBlock(media, className, declarations)}`;
};
const appendBlock = (source, className, declarations) => {
    const indented = declarations
        .split('\n')
        .map((line) => (line.trim().length > 0 ? `  ${line.trim()}` : ''))
        .filter((line) => line.length > 0)
