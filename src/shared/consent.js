 ...
 */
export const hasCurrentConsent = (settings) => settings.sentryOptIn !== null && settings.consentVersion >= CONSENT_VERSION;
/** True only when the user opted IN under the current wording. */
export const isOptedIn = (settings) => settings.sentryOptIn === true && settings.consentVersion >= CONSENT_VERSION;
