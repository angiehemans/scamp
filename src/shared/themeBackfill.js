...
import { BROWSER_RESET_BLOCK, BROWSER_RESET_SENTINEL, DEFAULT_BODY_FONT_FAMILY, LINK_CURSOR_BLOCK, LINK_CURSOR_SENTINEL, LIST_PADDING_BLOCK, LIST_PADDING_SENTINEL, } from './agentMd';
/**
 ...
 */
export const backfillThemeDefaults = (css) => {
    let root;
    try {
...
const checkHasFontSansToken = (root) => {
    let found = false;
    root.walkRules((rule) => {
        if (rule.selector.trim() !== ':root')
            return;
        rule.walkDecls((decl) => {
            if (decl.prop === '--font-sans')
                found = true;
        });
    });
    return found;
};
const checkHasBoxSizingReset = (root) => {
...
const checkHasBodyFontFamily = (root) => {
    let found = false;
...
const insertFontSansToken = (root) => {
    ...
    if (lastImportIndex === -1) {
        ...
        if (after)
            root.insertAfter(after, newRoot);
    }
};
const insertBoxSizingReset = (root) => {
