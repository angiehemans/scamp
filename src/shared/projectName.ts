...
export const validateProjectName = (raw: unknown): ValidationResult => {
  ...
  if (!PATTERN.test(trimmed)) {
    return {
      ok: false,
      error:
        'Use lowercase letters, numbers, and hyphens only. Cannot start or end with a hyphen.',
    };
  }
 ...
 */
export const suggestProjectName = (raw: string): string => {
  return raw
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, '-')
