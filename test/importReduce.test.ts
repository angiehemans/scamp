...
describe('grid templates — used values are not decisions', () => {
  ...
  const gridStyles = (over: Record<string, string>, w = 1100): Record<string, string> => ({
    display: 'grid',
    width: `${w}px`,
    ...over,
  });

  const cssOf = (root: CapturedNode): string => {
...
describe('resolveInheritance — typography reaching the words', () => {
  ...
  const styleOf = (root: CapturedNode, path: number[]): Record<string, string> => {
    let at = resolveInheritance(root);
    for (const i of path) at = at.children[i] as CapturedNode;
    return at.styles;
  };
