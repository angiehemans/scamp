...
describe('resolveElementAtState', () => {
  ...
  it('handles all three state keys independently', () => {
    ...
    expect(
      resolveElementAtState(el, 'desktop', DEFAULT_BREAKPOINTS, 'hover')
        .backgroundColor
    ).toBe('#aaaaaa');
    expect(
      resolveElementAtState(el, 'desktop', DEFAULT_BREAKPOINTS, 'active')
        .backgroundColor
    ).toBe('#888888');
    expect(
      resolveElementAtState(el, 'desktop', DEFAULT_BREAKPOINTS, 'focus')
        .backgroundColor
    ).toBe('#cccccc');
  });
});
