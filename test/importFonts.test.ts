...
const el = (id: string, fontFamily?: string): ScampElement => ({
  ...DEFAULT_RECT_STYLES,
  id,
  type: 'text',
...
const tree = (...els: ScampElement[]): Record<string, ScampElement> =>
  Object.fromEntries(els.map((e) => [e.id, e]));
...
describe('fontsNeededBy', () => {
  it('counts each family by how many elements want it', () => {
    const needs = fontsNeededBy(
      tree(
        el('a', 'Fraunces, serif'),
        el('b', 'Fraunces, serif'),
        el('c', 'Inter, sans-serif')
      )
    );
  ...
  it('counts one family however it is spelled in the stack', () => {
    const needs = fontsNeededBy(
      tree(el('a', 'Inter, sans-serif'), el('b', '"Inter", Helvetica, sans-serif'))
    );
  ...
  it('orders by use, since the headline face is rarely the commonest', () => {
    const needs = fontsNeededBy(
      tree(el('a', 'Rare, serif'), el('b', 'Common, serif'), el('c', 'Common, serif'))
    );
