...
import {
  ...
  rangeCovers,
  unicodeRangeOf,
} from '../src/renderer/src/lib/embedFonts';
...
describe('familiesUsedIn', () => {
  it('collects every family on the subtree, not just the root', () => {
    ...
    const found = familiesUsedIn(document.getElementById('r') as Element);
  ...
  it('keeps the whole stack, since a fallback can be a real face too', () => {
    ...
    expect(familiesUsedIn(document.getElementById('r') as Element).has('georgia')).toBe(true);
  });
});
