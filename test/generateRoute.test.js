...
describe('generateRouteTsx', () => {
    it('returns the samples from load() and renders the view with them, client when a view has events', () => {
        expect(generateRouteTsx({ viewName: 'Lobby', slug: 'lobby', props: lobbyProps, hasDatabase: false })).toBe(`import type { LoadContext, RouteProps } from 'scampjs/runtime';
...
`);
    ...
    it('writes the simplest static route for a view with no props', () => {
        expect(generateRouteTsx({ viewName: 'Home', slug: 'home', props: [], hasDatabase: false })).toBe(`import Home from '@/views/Home/Home';
