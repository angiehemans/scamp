...
import type { HistoryEntry } from '../src/renderer/store/historyTypes';
...
const at = (hhmmss: string): number => Date.parse(`2026-05-01T${hhmmss}.000Z`);
...
describe('mergeHistoryTimeline', () => {
  it('interleaves snapshots and undo entries newest-first by timestamp', () => {
    ...
    expect(
      result.map((i) => (i.kind === 'snapshot' ? i.snapshot.id : i.entry.id))
    ).toEqual(['h_draw2', 'snap_auto', 'h_draw1', 'snap_open']);
  });
  ...
  it('marks the cursor entry current and later entries future', () => {
    ...
    const flags = mergeHistoryTimeline([], entries, 1)
      .filter((i): i is Extract<typeof i, { kind: 'undo' }> => i.kind === 'undo')
  ...
  it('keeps the undo index pointing at the original stack position', () => {
    ...
    const byId = new Map(
      mergeHistoryTimeline([], entries, 2)
        .filter((i): i is Extract<typeof i, { kind: 'undo' }> => i.kind === 'undo')
