...
import { buildHtmlExport, THEME_FILE } from '@lib/htmlExport';
...
const fileAt = (files, path) => {
    const found = files.find((f) => f.path === path);
    if (!found)
        throw new Error(`no exported file at ${path}`);
    return found;
};
const simpleHome = () => sourceFor('home', [
    makeRoot(['a1b2']),
...
describe('buildHtmlExport — documents', () => {
    ...
    it('links the theme at the right depth from each page', () => {
        ...
        expect(fileAt(files, 'about/index.html').contents).toContain('href="../theme.css"');
    });
    it('links each page stylesheet as a plain sibling filename', () => {
        ...
        expect(fileAt(files, 'about/index.html').contents).toContain('href="index.css"');
    });
...
describe('buildHtmlExport — links', () => {
    ...
    it('rewrites a route link to the exported file', () => {
        ...
        expect(fileAt(files, 'index.html').contents).toContain('href="about/index.html"');
    });
...
describe('buildHtmlExport — assets', () => {
    const withImage = (pageName) => sourceFor(pageName, [
        makeRoot(['i1']),
    ...
    it('makes an image path relative on the home page', () => {
        ...
        expect(fileAt(files, 'index.html').contents).toContain('src="assets/hero.webp"');
    });
    it('climbs out of the directory for a nested page', () => {
        ...
        expect(fileAt(files, 'about/index.html').contents).toContain('src="../assets/hero.webp"');
    });
    it('keeps theme urls root-relative, since css resolves against the stylesheet', () => {
        ...
        expect(fileAt(files, THEME_FILE).contents).toContain('url(assets/font.woff2)');
    });
});
describe('buildHtmlExport — components', () => {
    const sidebarRow = () => sourceFor('SidebarRow', [
        makeRoot(['d3e4']),
        makeEl('d3e4', {
            type: 'text',
            name: 'label',
    ...
    const pageWithInstances = () => sourceFor('home', [
        makeRoot(['i1', 'i2']),
        makeEl('i1', {
