...
import { isSvgMarkup, sanitizeSvg, sanitizeSvgInner, prepareSvgForInsert, extractSvgColors, replaceSvgColor, } from '../src/renderer/src/lib/svg';
...
describe('prepareSvgForInsert', () => {
    ...
    it('keeps an explicit viewBox verbatim', () => {
        const result = prepareSvgForInsert('<svg viewBox="0 0 24 24" width="48" height="48"><rect/></svg>');
        expect(result.viewBox).toBe('0 0 24 24');
    });
    it('synthesizes a viewBox from intrinsic size when the source omits one', () => {
        ...
        const result = prepareSvgForInsert('<svg width="32" height="20"><rect/></svg>');
...
describe('extractSvgColors', () => {
    it('collects unique concrete colours from fill and stroke attributes', () => {
        const { colors, hasCurrentColor } = extractSvgColors('<path fill="#ff0000" stroke="#00ff00"/><circle fill="#ff0000"/>');
        expect(colors).toEqual(['#ff0000', '#00ff00']);
        expect(hasCurrentColor).toBe(false);
    });
    it('reads colours from inline style properties', () => {
        const { colors } = extractSvgColors('<path style="fill:#123456;stroke:#abcdef"/>');
        expect(colors).toEqual(['#123456', '#abcdef']);
    });
    it('includes gradient stop-color values', () => {
        const { colors } = extractSvgColors('<defs><linearGradient><stop stop-color="#111111"/><stop stop-color="#222222"/></linearGradient></defs>');
        expect(colors).toEqual(['#111111', '#222222']);
    });
    it('flags currentColor separately and skips none / url() paints', () => {
        const { colors, hasCurrentColor } = extractSvgColors('<path fill="currentColor" stroke="none"/><rect fill="url(#grad)"/>');
        expect(colors).toEqual([]);
        expect(hasCurrentColor).toBe(true);
    });
    it('dedupes case-insensitively but keeps the first-seen spelling', () => {
        const { colors } = extractSvgColors('<path fill="#ABCDEF"/><path fill="#abcdef"/>');
...
describe('replaceSvgColor', () => {
    it('rewrites every occurrence across attributes', () => {
        const out = replaceSvgColor('<path fill="#ff0000"/><circle stroke="#ff0000"/>', '#ff0000', '#0000ff');
    ...
    it('rewrites a colour inside an inline style', () => {
        const out = replaceSvgColor('<path style="fill:#111111;stroke:#222222"/>', '#111111', '#999999');
    ...
    it('leaves unrelated colours and none untouched', () => {
        const out = replaceSvgColor('<path fill="#ff0000" stroke="none"/><circle fill="#00ff00"/>', '#ff0000', '#0000ff');
