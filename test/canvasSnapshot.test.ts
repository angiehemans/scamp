...
import {
  answerSnapshotTool,
  CANVAS_STATE_ELEMENT_LIMIT,
  componentPathsRelative,
  getActiveTarget,
  getCanvasState,
  getElementById,
  getElementTree,
...
const build = (over: Partial<SnapshotInput> = {}): SnapshotInput => ({
...
describe('getSelectedElement', () => {
  ...
  it('includes the user-set name only when there is one', () => {
    ...
    expect(getSelectedElement(build({ selectedIds: ['a1b2'] }))).not.toHaveProperty(
      'name'
    );
  });
  ...
  it('reports the root with a null parentId', () => {
    expect(getSelectedElement(build({ selectedIds: [ROOT_ELEMENT_ID] }))).toMatchObject(
      { parentId: null }
    );
  });
});
...
describe('pagePathsRelative', () => {
  ...
  it('never emits an absolute path', () => {
    ...
    // would go with it.
    for (const format of ['nextjs', 'legacy'] as const) {
...
describe('getCanvasState', () => {
  ...
  it('survives an empty canvas with no project open', () => {
    const out = getCanvasState(
      build({ target: null, elements: {}, selectedIds: [] })
    );
...
describe('answerSnapshotTool', () => {
  ...
  it('throws on an unrouted tool rather than returning null', () => {
    ...
    expect(() => answerSnapshotTool('scamp_made_up', {}, build())).toThrow(
      /Unknown snapshot tool/
    );
  });
});
...
describe('getViewProps', () => {
  const lobby = (): Record<string, ScampElement> => ({
    [ROOT_ELEMENT_ID]: el({
      id: ROOT_ELEMENT_ID,
      parentId: null,
