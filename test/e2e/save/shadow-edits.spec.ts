...
/**
 * Phase 1 of docs/plans/incremental-writes-plan.md runs beside every
 * real save. This proves it is wired to that path rather than only
 * unit-tested, and that a canvas edit really does touch far fewer lines
 * than the whole-file write rewrites.
 */
...
const totals = (window: Parameters<typeof pageRoot>[0]): Promise<Totals> =>
  window.evaluate(
    () => (globalThis as unknown as { __scampShadowEdits: () => Totals }).__scampShadowEdits()
  ) as Promise<Totals>;

test('a canvas edit derives edits that reproduce the save and touch a fraction of it', async ({
  window,
}) => {
