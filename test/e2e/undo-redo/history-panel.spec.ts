...
test.describe('history panel', () => {
  test('empty state reads "No changes made in this session"', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await window.getByRole('tab', { name: 'History' }).click();
    await expect(
      window.getByText('No changes made in this session')
    ).toBeVisible();
  });

  test('drawing a rectangle adds a "Drew rectangle" entry', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('clicking an older entry restores the canvas to that point', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('new edit after jumping back discards the forward entries', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
