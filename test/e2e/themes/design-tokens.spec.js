...
test.describe('themes: spacing / border / radius / shadow tokens', () => {
    test('adding default spacing + radius writes grouped tokens to theme.css', async ({ window, project, }) => {
    ...
    test('adding a single shadow token then renaming its label round-trips', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await expect(panel).toBeVisible();
        ...
        expect(await project.readTheme()).toMatch(/--shadow-md:\s*0 4px 6px rgba\(0, 0, 0, 0\.07\), 0 2px 4px rgba\(0, 0, 0, 0\.06\)/);
    });
    test('the theme section nav lists and scroll-jumps to the new sections', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        ...
        await expect(window.locator('[data-theme-section="shadow"]')).toBeInViewport();
    });
});
