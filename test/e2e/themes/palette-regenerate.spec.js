...
test.describe('themes: palette regenerate', () => {
    test('Generate keeps the palette in place (no jump to the bottom)', async ({ window, }) => {
        ...
        const orderBefore = await palettes.evaluateAll((els) => els.map((e) => e.getAttribute('data-palette')));
        ...
        const orderAfter = await palettes.evaluateAll((els) => els.map((e) => e.getAttribute('data-palette')));
