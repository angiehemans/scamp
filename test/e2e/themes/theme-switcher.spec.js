...
test.describe('themes: theme switcher', () => {
    ...
    test('editing a semantic in the Dark block writes to .dark, not :root', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await expect(panel).toBeVisible();
    ...
    test('removing a theme drops its block from theme.css', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await panel.getByTestId('add-theme').click();
    ...
    test('the canvas toolbar switcher appears and toggles the previewed theme', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
