...
test.describe('themes: DESIGN.md generation', () => {
    test('generates DESIGN.md from the theme tokens on project open', async ({ window, project, }) => {
    ...
    test('regenerates token YAML while preserving authored prose', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        ...
        await fs.writeFile(path.join(project.dir, 'DESIGN.md'), `---
name: My App
...
`, 'utf-8');
    ...
    test('editing a Documentation form field writes prose to DESIGN.md', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
    ...
    test('an external DESIGN.md edit populates the Documentation forms', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await expect
...
`, 'utf-8');
        ...
        await expect(panel.locator('[data-design-doc-section="Layout"]')).toHaveValue('Grid guidance from an agent.');
    });
});
