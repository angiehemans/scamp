...
test.describe('themes: panel CRUD', () => {
    ...
    test('deleting a token removes it from theme.css', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
    ...
    test('the section nav scroll-jumps the editor to a section', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
    ...
    test('the token list scrolls instead of spilling out when there are many tokens', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await expect(panel).toBeVisible();
        ...
        if (!dialogBox)
            throw new Error('dialog has no bounding box');
...
test.describe('themes: panel is read-only in legacy projects', () => {
    ...
    test('shows a migration notice instead of the token editor', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="design-system"]').click();
        const panel = window.getByTestId('theme-panel');
        await expect(panel).toBeVisible();
