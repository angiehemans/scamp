...
const openPanel = async (window: Page): Promise<Locator> => {
 ...
 */
const valueField = (scope: Locator, tokenName: string): Locator =>
  scope
...
test.describe('themes: a semantic token can take a literal value', () => {
  test('typing a hex writes the literal, not a var() reference', async ({
    window,
    project,
  }) => {
  ...
  test('the swatch still opens the picker for the primitives', async ({
    window,
  }) => {
    const panel = await openPanel(window);
    ...
    await expect(
      window.getByRole('button', { name: 'Tokens', exact: true })
    ).toBeVisible();
  });

  test('a literal in a theme block writes to that block, not :root', async ({
    window,
    project,
  }) => {
    const panel = await openPanel(window);
