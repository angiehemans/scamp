...
test.describe('themes: text styles', () => {
    test('adding default text styles writes the --text-<style>-<prop> token groups', async ({ window, project, }) => {
    ...
    test('applying a text style links the element to the style tokens', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        ...
        if (!className)
            throw new Error('no text element created');
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*font-size:\\s*var\\(--text-h1-size\\)`, 's'));
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*line-height:\\s*var\\(--text-h1-leading\\)`, 's'));
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*font-weight:\\s*700`, 's'));
    });
});
