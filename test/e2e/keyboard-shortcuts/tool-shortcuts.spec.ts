...
test.describe('keyboard shortcuts: tool shortcuts', () => {
  const TOOLS: ReadonlyArray<{ key: 'v' | 'r' | 't' | 'i' | 'f'; tool: string }> = [
    { key: 'v', tool: 'select' },
  ...
  for (const { key, tool } of TOOLS) {
    test(`${key.toUpperCase()} activates the ${tool} tool`, async ({ window }) => {
      ...
      await expect(window.getByTestId('element-toolbar')).toHaveAttribute(
        'data-active-tool',
        tool
      );
    });
  ...
  test('tool shortcuts do not fire while a text input is focused', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
