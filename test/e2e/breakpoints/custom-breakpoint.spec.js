...
import { canvasSizeButton, canvasSizePopover, closeProjectSettings, openProjectSettings, switchBreakpoint, } from '../fixtures/breakpoints';
...
test.describe('breakpoints: custom breakpoint', () => {
    test('adding a custom breakpoint surfaces it in the canvas-size popover', async ({ window, }) => {
        ...
        await expect(canvasSizePopover(window).getByRole('button', { name: /Custom/ })).toBeVisible();
    });
    test('edits at a custom breakpoint emit its @media (max-width) block', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        ...
        const mediaMatch = css.match(/@media \(max-width: 600px\)\s*\{([\s\S]*?)\n\}/);
