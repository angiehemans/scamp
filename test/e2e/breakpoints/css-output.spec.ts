...
test.describe('breakpoints: CSS output', () => {
  test('Tablet + Mobile overrides emit widest-first @media blocks', async ({
    window,
    project,
  }) => {
  ...
  test('unknown @media queries round-trip verbatim', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(finalCss).toMatch(
      new RegExp(`\\.${className}\\s*\\{[^}]*padding:\\s*18px`)
    );
