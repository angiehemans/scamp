...
export const canvasSizeButton = (page: Page): Locator =>
  page.locator('[data-testid="canvas-size-button"]');

/** The canvas-size popover (role=dialog). */
export const canvasSizePopover = (page: Page): Locator =>
  page.locator('[data-testid="canvas-size-popover"]');
 ...
 */
export const switchBreakpoint = async (
  page: Page,
  bpId: 'desktop' | 'tablet' | 'mobile' | string,
  visibleLabel: string
): Promise<void> => {
  ...
  await canvasSizePopover(page)
    ...
    .first()
    .click();
  await expect(canvasSizeButton(page)).toHaveAttribute(
...
export const closeProjectSettings = async (page: Page): Promise<void> => {
