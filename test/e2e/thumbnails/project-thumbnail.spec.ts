...
const thumbnailPath = (projectDir: string): string =>
  path.join(projectDir, '.scamp', 'preview.png');

/** Wait for the debounced capture to land, then read it. */
const waitForThumbnail = async (projectDir: string): Promise<Buffer> => {
...
test.describe('start-screen project thumbnail', () => {
  test('a home-page save writes a thumbnail with the page actually in it', async ({
    window,
    project,
  }) => {
  ...
  test('capturing does not disturb the canvas the user is looking at', async ({
    window,
    project,
  }) => {
    await expect(canvasElement(window, 'rect_swatch')).toBeVisible();
    ...
    await canvasFrame(window).evaluate((frame) => {
      ...
      const observer = new MutationObserver(() => {
        seen.push((frame as HTMLElement).style.transform);
      });
      observer.observe(frame, {
        attributes: true,
        attributeFilter: ['style'],
      });
      (globalThis as unknown as { __transforms: string[] }).__transforms = seen;
    });

    const before = await canvasFrame(window).evaluate(
      (frame) => (frame as HTMLElement).style.transform
    );
    ...
    const transforms = await window.evaluate(
      () => (globalThis as unknown as { __transforms: string[] }).__transforms
    );
...
test.describe('start-screen project thumbnail: framework format', () => {
  ...
  test('a home-view save writes a thumbnail with the view actually in it', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
...
test.describe('start-screen project thumbnail: paint that has to be inlined', () => {
  ...
  const GREEN_PNG = Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAYAAABytg0kAAAAFUlEQVR4nGNk+M/wn4GBgYkBBEAMADevAgv1mn5oAAAAAElFTkSuQmCC',
    'base64'
  );
...
`,
  ...
  test('inlines a background image so it survives the rasterise', async ({
    window,
    project,
  }) => {
