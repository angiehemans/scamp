...
test.describe('routes in project settings', () => {
    ...
    test('lists every route, including API handlers, and sets a render mode', async ({ window, project, }) => {
    ...
    test('opening a route leaves settings and shows it in the code panel', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await window.locator('[data-section="settings"]').click();
        await window
