...
test.describe('settings: artboard background', () => {
    test('artboard color lives in scamp.config.json, not in the page CSS', async ({ window, project, }) => {
        ...
        await expect
            .poll(async () => fs.readFile(path.join(project.dir, 'scamp.config.json'), 'utf-8'), { timeout: 5_000 })
            .toContain('#abcdef');
