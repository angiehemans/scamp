...
const readConfig = async (projectDir: string): Promise<{ breakpoints: Array<{ id: string; label: string; width: number }> }> => {
  const raw = await fs.readFile(path.join(projectDir, 'scamp.config.json'), 'utf-8');
  return JSON.parse(raw) as { breakpoints: Array<{ id: string; label: string; width: number }> };
};

test.describe('settings: project breakpoints editor', () => {
  test('adding a breakpoint writes it to scamp.config.json and list stays widest-first', async ({
    window,
    project,
  }) => {
  ...
  test('Desktop breakpoint remove button is disabled', async ({ window }) => {
    ...
    await expect(
      window.getByRole('button', { name: /^Remove Desktop \(disabled\)/ })
    ).toBeDisabled();
  });

  test('removing a non-desktop breakpoint shrinks the list', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
