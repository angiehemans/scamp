...
test.describe('migrate a Next.js project to the Scamp framework', () => {
    ...
    test('converts the open page when the canvas has unsaved changes', async ({ window, project, }) => {
        ...
        await window.getByRole('button', { name: 'Migrate', exact: true }).click();
        await expect(notice).toBeHidden({ timeout: 30_000 });
        const exists = (file) => fs.access(path.join(project.dir, file)).then(() => true, () => false);
