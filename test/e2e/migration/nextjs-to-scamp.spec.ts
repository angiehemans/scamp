...
const markup = (tsx: string): string => {
...
test.describe('migrate a Next.js project to the Scamp framework', () => {
  ...
  test('turns pages into views and routes, moves the theme, swaps the stack, and reports the rest', async ({
    window,
    project,
  }) => {
    ...
    await fs.writeFile(
      path.join(project.dir, 'app', 'api', 'ping', 'route.ts'),
    ...
    expect(markup(homeView).replace('className={`${styles.root} ${className ?? \'\'}`}', 'className={styles.root}')).toBe(
      markup(HOME_TSX)
    );
