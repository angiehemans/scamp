...
const writeThemeCss = async (projectDir, themeCss) => {
    await fs.writeFile(path.join(projectDir, 'theme.css'), themeCss, 'utf-8');
};
test.describe('properties panel: spacing token picker', () => {
    test('picker icon applies a length token to all four sides of padding', async ({ window, project, }) => {
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*padding:\\s*var\\(--space-md\\)`, 's'));
    });
    test('typed px value then token swap preserves token form', async ({ window, project, }) => {
        await writeThemeCss(project.dir, `:root {\n  --color-text: #111;\n  --space-sm: 8px;\n  --space-md: 16px;\n}\n`);
        await expect(pageRoot(window)).toBeVisible();
    ...
    test('empty token list shows the Add token button', async ({ window, project, }) => {
