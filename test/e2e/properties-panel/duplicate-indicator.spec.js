...
test.describe('properties panel: duplicate CSS indicator', () => {
    ...
    test('flags a duplicate even when it does not change the winning value', async ({ window, project, }) => {
        ...
        if (!block)
            throw new Error('no class block');
        const existingHeight = block[1]?.match(/height:\s*[^;]+;/)?.[0];
        if (!existingHeight)
            throw new Error('no height to duplicate');
        const withDuplicate = original.replace(new RegExp(`(\\.${className}\\s*\\{)`), `$1\n  ${existingHeight}`);
        expect(withDuplicate).not.toBe(original);
        await fs.writeFile(cssPath, withDuplicate, 'utf-8');
        await expect(panelSection(window, 'Size').getByTestId('duplicate-dot')).toBeVisible({ timeout: 10_000 });
    });
    test('the user can still save after a duplicate-only external edit', async ({ window, project, }) => {
        ...
        if (!existingHeight)
            throw new Error('no height to duplicate');
        await fs.writeFile(cssPath, original.replace(new RegExp(`(\\.${className}\\s*\\{)`), `$1\n  ${existingHeight}`), 'utf-8');
        await expect(panelSection(window, 'Size').getByTestId('duplicate-dot')).toBeVisible({ timeout: 10_000 });
