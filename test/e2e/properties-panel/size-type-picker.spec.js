...
test.describe('properties panel: size type picker', () => {
    ...
    test('typing a number then picking Percent emits width in %', async ({ window, project, }) => {
        const className = await drawRect(window);
    ...
    test('picking Hug (on a flex container) emits width: fit-content', async ({ window, project, }) => {
        const className = await drawRect(window);
    ...
    test('arrow keys step the value (±1, Shift ±10)', async ({ window, project, }) => {
        const className = await drawRect(window);
