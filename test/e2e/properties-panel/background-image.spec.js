...
test.describe('properties panel: background image', () => {
    test('uploading a background image writes url() + size/position/repeat', async ({ window, app, project, }) => {
    ...
    test('"Remove background image" clears all four background props', async ({ window, app, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const fixturePath = await writeFixtureImage(project.dir, 'bg.png');
