...
test.describe('properties panel: filters', () => {
    test('"+ Add filter" emits a default filter shorthand', async ({ window, project, }) => {
        ...
        // Default: blur(4px).
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*filter:\\s*blur\\(4px\\)`, 's'));
    });
    test('multiple filters emit a space-joined list', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
    ...
    test('changing the kind dropdown resets the value to that kind\'s default', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*filter:\\s*brightness\\(100%\\)`, 's'));
    });
    test('changing the value input rewrites the filter argument', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
        ...
        await waitForSaved(window);
        const { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*filter:\\s*blur\\(20px\\)`, 's'));
    });
    test('remove button drops a filter row from the CSS', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
    ...
    test('all filter rows removed drops the declaration entirely', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
    ...
    test('enabling the backdrop toggle reveals a separate add button', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const filters = panelSection(window, 'Filters');
        ...
        await waitForSaved(window);
        const { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*backdrop-filter:\\s*blur\\(4px\\)`, 's'));
