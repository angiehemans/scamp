...
test.describe('properties panel: shadows', () => {
  test('"+ Add shadow" emits a default box-shadow shorthand', async ({
    window,
    project,
  }) => {
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}[^}]*box-shadow:\\s*0px 4px 8px rgba\\(0, 0, 0, 0\\.15\\)`,
        's'
      )
    );
  });

  test('multiple shadows emit a comma-separated list', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('inset toggle prepends `inset` to the shadow', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*box-shadow:\\s*inset\\s+`, 's')
    );
  });

  test('opacity field rewrites the alpha component', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}[^}]*box-shadow:[^;]*rgba\\(0, 0, 0, 0\\.5\\)`,
        's'
      )
    );
  });

  test('remove button drops a shadow row from the CSS', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
