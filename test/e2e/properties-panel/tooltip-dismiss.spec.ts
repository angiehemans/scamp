...
test.describe('properties panel: tooltip dismissal', () => {
  test('clicking a size field with the pointer parked opens no tooltip', async ({
    window,
  }) => {
  ...
  test('a hovered tooltip closes when the field is clicked', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 240, y: 200 });
  ...
  test('hovering still opens a tooltip, and Escape dismisses it', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 240, y: 200 });
