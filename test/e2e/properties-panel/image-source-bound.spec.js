...
test.describe('properties panel: image source away from a plain page', () => {
    test('Replace imports a file in the component editor, where activePage is null', async ({ app, window, project, }) => {
        ...
        await writeFile(picked, Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==', 'base64'));
        ...
        await expect(window.getByPlaceholder('Image description')).toHaveValue('original.png', { timeout: 10_000 });
    });
    test('a bound src shows the binding rather than an empty field', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
