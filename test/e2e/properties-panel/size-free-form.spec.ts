...
test.describe('properties panel: free-form size input', () => {
  test('typing 100vh stays Fixed and writes the verbatim value', async ({
    window,
    project,
  }) => {
  ...
  test('a bare number is treated as px', async ({ window, project }) => {
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*width:\\s*320px;`, 's')
    );
  });
  ...
  test('calc() and var() round-trip verbatim', async ({ window, project }) => {
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}[^}]*width:\\s*calc\\(100% - 20px\\);`,
        's'
      )
    );
    ...
    ({ css } = await readPageFiles(project.dir, project.pageName));
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*width:\\s*var\\(--page-w\\);`, 's')
    );
  });
});
