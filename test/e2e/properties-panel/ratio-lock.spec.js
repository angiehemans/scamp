...
const sizeLockToggle = (window) => panelSection(window, 'Size').getByRole('button', {
    name: /Lock ratio|Ratio locked/,
});
test.describe('properties panel: aspect-ratio lock', () => {
    test('locking then changing width recomputes height to hold the ratio', async ({ window, project, }) => {
    ...
    test('switching an axis to stretch auto-releases the lock', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
    ...
    test('locking disables the edge resize handles, leaving only corners', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 300, y: 220 });
