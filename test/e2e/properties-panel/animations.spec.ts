...
test.describe('properties panel: animations', () => {
  test('picking a preset writes animation shorthand + @keyframes block', async ({
    window,
    project,
  }) => {
    ...
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:\\s*fade-in-up`, 's')
    );
    // And the page-level @keyframes block exists.
    expect(css).toMatch(/@keyframes\s+fade-in-up\s*\{/);
  });

  test('switching presets updates the animation name in CSS', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:\\s*pulse`, 's')
    );
    ...
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:\\s*spin`, 's')
    );
  });

  test('setting None via the Remove button drops the animation declaration', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('Duration field rewrites the animation timing', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:\\s*pulse\\s+1500ms`, 's')
    );
  });

  test('Easing / Delay / Direction / Fill-mode flow into the shorthand', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    await waitForSaved(window);
    const { css } = await readPageFiles(project.dir, project.pageName);
    const shorthand = css.match(
      new RegExp(`\\.${className}[^}]*animation:\\s*([^;]+);`, 's')
    )?.[1];
  ...
  test('Play state segmented control toggles running ↔ paused', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    let { css } = await readPageFiles(project.dir, project.pageName);
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:[^;]*paused`, 's')
    );
  ...
  test('per-state hover animation emits :hover { animation: ... }', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}:hover\\s*\\{[^}]*animation:\\s*shake`,
        's'
      )
    );
  ...
  test('Iteration popover toggles between Number and Infinite', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    let { css } = await readPageFiles(project.dir, project.pageName);
    expect(css).toMatch(
      new RegExp(`\\.${className}[^}]*animation:[^;]*infinite`, 's')
    );
  ...
  test('▶ Play preview button is present and click does not error', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
