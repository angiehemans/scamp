...
test.describe('properties panel: CSS mode', () => {
    test('switching to CSS mode and editing declarations writes to disk', async ({ window, project, }) => {
    ...
    test('Cmd+S commits two sequential CSS edits without losing the first', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        await setPanelMode(window, 'CSS');
