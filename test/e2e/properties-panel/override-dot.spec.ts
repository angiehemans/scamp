...
test.describe('properties panel: override indicator dot', () => {
  test('no dot at desktop even when base styles are set', async ({ window }) => {
    ...
    await expect(
      panelSection(window, 'Spacing').locator('[data-testid="override-dot"]')
    ).toHaveCount(0);
  });

  test('dot appears next to Spacing after a Tablet-scoped padding edit', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
    ...
    const dot = panelSection(window, 'Spacing').locator(
      '[data-testid="override-dot"]'
    );
    await expect(dot).toBeVisible();
    // Other sections still have no dot — nothing overridden there.
    await expect(
      panelSection(window, 'Border').locator('[data-testid="override-dot"]')
    ).toHaveCount(0);
  });

  test('right-clicking the dot resets the section overrides at the active breakpoint', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(`\\.${className}\\s*\\{[^}]*padding:\\s*24px`)
    );
