...
test.describe('properties panel: shadow preset', () => {
  test('picking a shadow preset replaces the shadow rows with the token value', async ({
    window,
    project,
  }) => {
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}[^}]*box-shadow:\\s*0px 4px 6px rgba\\(0, 0, 0, 0\\.07\\), 0px 2px 4px rgba\\(0, 0, 0, 0\\.06\\)`,
        's'
      )
    );
  });
