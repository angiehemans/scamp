...
test.describe('properties panel: flex alignment grid', () => {
    ...
    test('clicking the bottom-right cell emits flex-end on both axes', async ({ window, project, }) => {
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*justify-content:\\s*flex-end`, 's'));
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*align-items:\\s*flex-end`, 's'));
    });
    test('clicking the middle-center cell emits center on both axes', async ({ window, project, }) => {
        const className = await setFlexRow(window);
        await panelSection(window, 'Layout')
        ...
        await waitForSaved(window);
        const { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*justify-content:\\s*center`, 's'));
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*align-items:\\s*center`, 's'));
    });
    test('double-clicking the grid distributes with space-between', async ({ window, project, }) => {
        const className = await setFlexRow(window);
        await panelSection(window, 'Layout')
        ...
        await waitForSaved(window);
        const { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*justify-content:\\s*space-between`, 's'));
    });
});
