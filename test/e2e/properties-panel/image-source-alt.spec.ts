...
test.describe('properties panel: image source and alt', () => {
  test('typing alt text writes it to the TSX intact', async ({
    window,
    project,
  }) => {
  ...
  test('an absolute URL can be typed into Source', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('a project-relative path can be typed into Source', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
...
test.describe('properties panel: image section placement', () => {
  test('Image sits directly after Element, ahead of the style sections', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await layersRowByClass(window, 'image_a1b2').click();
    ...
    const titles = await propertiesPanel(window)
      ...
      .evaluateAll((els) =>
        els.map((el) => el.getAttribute('data-panel-section'))
      );
  ...
  test('Background offers no "Set background image" for an img', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await layersRowByClass(window, 'image_a1b2').click();
    ...
    await expect(
      background.getByRole('button', { name: 'Set background image' })
    ).toHaveCount(0);
  });
});
...
test.describe('properties panel: non-image elements are unaffected', () => {
  test('a rect keeps its "Set background image" button', async ({ window }) => {
    ...
    await expect(
      panelSection(window, 'Background').getByRole('button', {
