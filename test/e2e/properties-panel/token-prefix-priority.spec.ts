...
const seedTheme = async (dir: string): Promise<void> => {
...
const firstTokenOption = (window: Parameters<typeof pageRoot>[0]) =>
  window.getByRole('option', { name: /^--/ }).first();

test.describe('properties panel: token prefix prioritization', () => {
  test('border-width picker floats --border tokens to the top', async ({
    window,
    project,
  }) => {
  ...
  test('radius picker floats --radius tokens to the top', async ({
    window,
    project,
  }) => {
    await seedTheme(project.dir);
  ...
  test('padding picker floats --space tokens to the top', async ({
    window,
    project,
  }) => {
    await seedTheme(project.dir);
