...
const rootBlock = (css: string): string => {
  ...
  if (start === -1) throw new Error(`no .root block in:\n${css}`);
  return css.slice(start, css.indexOf('}', start) + 1);
};

test.describe('REPRO: element edits leaking onto the root', () => {
  test('setting gap on a flex child does not add gap to .root', async ({
    window,
    project,
  }) => {
    ...
    expect(css, 'element should have the gap').toMatch(
      new RegExp(`\\.${className}[^}]*gap:\\s*24px`, 's')
    );
    expect(rootBlock(css), 'root must NOT have gained a gap').not.toContain('gap');
  });

  test('setting padding on a child does not add padding to .root', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css, 'element should have the padding').toMatch(
      new RegExp(`\\.${className}[^}]*padding:\\s*16px`, 's')
    );
    expect(rootBlock(css), 'root must NOT have gained padding').not.toContain(
      'padding'
    );
  });

  test('the root keeps its own values when a child is edited', async ({
    window,
    project,
  }) => {
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css, 'child gets its own gap').toMatch(
      new RegExp(`\\.${className}[^}]*gap:\\s*40px`, 's')
    );
    expect(rootBlock(css), 'root keeps ITS gap, not the child edit').toContain(
      'gap: 8px'
    );
  });

  test('padding set at a non-desktop breakpoint does not reach .root', async ({
    window,
    project,
  }) => {
    ...
    const { css } = await readPageFiles(project.dir, project.pageName);
    expect(css, 'child gets the tablet override').toMatch(
      new RegExp(`\\.${className}[^}]*padding:\\s*12px`, 's')
    );
  ...
  test('editing a NESTED element does not reach .root', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const inner = await drawAndSelectRect(
      window,
      { x: 120, y: 120 },
      { x: 260, y: 240 }
    );
