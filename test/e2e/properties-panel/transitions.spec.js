...
import { drawAndSelectRect, panelInputByPrefix, panelSection, } from '../fixtures/panel';
import { pageRoot } from '../fixtures/selectors';
import { readPageFiles, waitForSaved } from '../fixtures/assertions';
test.describe('properties panel: transitions', () => {
    test('adding a transition writes a `transition` shorthand to disk', async ({ window, project, }) => {
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*transition:\\s*all 200ms ease;`, 's'));
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*transition:\\s*opacity 300ms ease;`, 's'));
    });
    test('multiple transitions emit a comma-separated shorthand and removing one shrinks it', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const transitions = panelSection(window, 'Transitions');
        ...
        await waitForSaved(window);
        let { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*transition:\\s*all 200ms ease, all 200ms ease;`, 's'));
