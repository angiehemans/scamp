...
test.describe('alignment grid: matches the design', () => {
    ...
    const layout = (window) => panelSection(window, 'Layout');
    const bars = (window) => layout(window).locator('[data-align-bar]');
    const dots = (window) => layout(window).locator('[data-align-dot]');
    const cell = (window, label) => layout(window).getByRole('button', { name: label });
    test('row: three bars at the designed sizes, in the chosen cell', async ({ window, }) => {
        ...
        const sizes = await Promise.all([0, 1, 2].map(async (i) => {
            const b = await boxOf(bars(window).nth(i));
            return `${Math.round(b.width)}x${Math.round(b.height)}`;
        }));
        expect(sizes).toEqual(['3x10', '3x14', '3x8']);
    });
    test('row: the bar group is centred in its cell, for every cell', async ({ window, }) => {
        ...
        for (const label of [
            ...
            expect(Math.abs(groupCentreX - centre(cellBox).x), `${label}: bar group centred horizontally in its cell`).toBeLessThan(TOLERANCE);
        }
    });
    test('row: the occupied cell hides its dot; the other eight remain', async ({ window, }) => {
        await setUp(window, 'Flex row');
        await cell(window, 'Align middle center').click();
        await expect(dots(window)).toHaveCount(8);
        await expect(layout(window).locator('[data-align-dot="1-1"]')).toHaveCount(0);
    });
    test('row: align start/center/end moves the bars within the band', async ({ window, }) => {
        ...
        expect(Math.abs(tall.y - short.y), 'start: tops share an edge').toBeLessThan(TOLERANCE);
        await cell(window, 'Align middle left').click();
        tall = await boxOf(bars(window).nth(1));
        short = await boxOf(bars(window).nth(2));
        expect(Math.abs(centre(tall).y - centre(short).y), 'center: centres share a line').toBeLessThan(TOLERANCE);
        await cell(window, 'Align bottom left').click();
        tall = await boxOf(bars(window).nth(1));
        short = await boxOf(bars(window).nth(2));
        expect(Math.abs(tall.y + tall.height - (short.y + short.height)), 'end: bottoms share an edge').toBeLessThan(TOLERANCE);
    });
    test('row: space-between spreads one bar per column', async ({ window }) => {
        ...
        for (const [i, label] of [
            ...
            const cellBox = await boxOf(cell(window, label));
            expect(Math.abs(centre(bar).x - centre(cellBox).x), `${label}: bar centred in its cell`).toBeLessThan(TOLERANCE);
        }
    });
    test('column: bars turn horizontal and stack, at the designed sizes', async ({ window, }) => {
    ...
    test('column: the bar group is centred in its cell', async ({ window }) => {
        ...
        for (const label of [
            ...
            const cellBox = await boxOf(cell(window, label));
            expect(Math.abs(groupCentreY - centre(cellBox).y), `${label}: bar group centred vertically in its cell`).toBeLessThan(TOLERANCE);
        }
    });
});
