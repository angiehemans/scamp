...
test.describe('properties panel: blend modes', () => {
    test('mix-blend-mode dropdown writes the keyword to CSS', async ({ window, project, }) => {
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*mix-blend-mode:\\s*multiply`, 's'));
    ...
    test('mix-blend-mode supports grouped values like color-burn / hard-light', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*mix-blend-mode:\\s*color-burn`, 's'));
    });
    test('background-blend-mode is hidden until both bg color and bg image are set', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        ...
        await expect(background.getByText('Blend', { exact: true })).toHaveCount(0);
    });
});
