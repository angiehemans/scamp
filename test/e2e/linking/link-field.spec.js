...
test.describe('linking: link field', () => {
    test('picking Page on a div rect converts it to <a> with the page href', async ({ window, project, }) => {
        ...
        expect(tsx).toMatch(new RegExp(`<a [^>]*data-scamp-id="${className}"[^>]*href="/"`));
    });
    test('typing an external URL converts to <a> and sets href', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const elementSection = panelSection(window, 'Element');
        ...
        await waitForSaved(window);
        const { tsx } = await readPageFiles(project.dir, project.pageName);
        expect(tsx).toMatch(new RegExp(`<a [^>]*data-scamp-id="${className}"[^>]*href="https://scampdesign\\.app"`));
    });
    test('"Open in new tab" toggle adds target + rel attributes', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const className = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 260, y: 200 });
        await waitForSaved(window);
        const elementSection = panelSection(window, 'Element');
        ...
        await waitForSaved(window);
        const { tsx } = await readPageFiles(project.dir, project.pageName);
        expect(tsx).toMatch(new RegExp(`<a [^>]*data-scamp-id="${className}"[^>]*target="_blank"[^>]*rel="noopener noreferrer"`));
    });
});
