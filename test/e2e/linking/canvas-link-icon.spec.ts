...
test.describe('linking: canvas chain icon', () => {
  test('linking an element shows a chain icon on the canvas', async ({
    window,
  }) => {
    ...
    await expect(
      window.getByRole('button', { name: /Opens https:\/\/example\.com/ })
    ).toBeVisible({ timeout: 10_000 });
  });

  test('a broken page link shows the broken warning in the panel', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const patched = tsx.replace(
      new RegExp(`(<div data-scamp-id="${className}"[^>]*)/>`),
      '<a data-scamp-id="' +
        className +
        '" className={styles.' +
        className +
    ...
    const warning = panelSection(window, 'Element').getByText(
      /Page\s+\/missing-page\s+doesn't exist/i
    );
