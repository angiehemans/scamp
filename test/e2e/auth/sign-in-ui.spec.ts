...
const toStartScreen = async (window: Parameters<typeof expect>[0] extends never ? never : any): Promise<void> => {
...
test.describe('start screen: account', () => {
  ...
  test('sits alongside the normal start screen, blocking nothing', async ({
    window,
  }) => {
