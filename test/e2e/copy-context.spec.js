...
const copyButton = (window) => window.locator('[data-action="copy-context"]');
/**
...
test.describe('copy context', () => {
    test('the toolbar offers the button, enabled with nothing selected', async ({ window, }) => {
    ...
    test('clicking copies the page context when nothing is selected', async ({ window, app, }) => {
        await expect(pageRoot(window)).toBeVisible();
    ...
    test('clicking names the selected element and its styles', async ({ window, app, }) => {
        await expect(pageRoot(window)).toBeVisible();
