...
test.describe('components sidebar — rename component', () => {
  test('renames the folder + flips function/type names + updates page imports', async ({
    window,
    project,
  }) => {
  ...
  test('no "Save failed" surfaces during rename (target-swap + pending-write regressions)', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
