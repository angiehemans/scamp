...
const panel = (page: Parameters<typeof measureFrame>[0]) =>
 ...
 */
const panelSection = (
  page: Parameters<typeof measureFrame>[0],
  title: string
) => panel(page).locator(`[data-panel-section="${title}"]`);

/** Drop a Card on empty canvas; it auto-selects on insert. */
const placeAndSelectCard = async (
  page: Parameters<typeof measureFrame>[0]
): Promise<void> => {
...
test.describe('components: properties panel for an instance', () => {
  test('titles the Element section with the component name', async ({
    window,
  }) => {
  ...
  test('shows Size and hides the sections a page cannot write', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await placeAndSelectCard(window);
  ...
  test('still shows the full section set for a plain rectangle', async ({
    window,
  }) => {
