...
import { clickContextMenuItem, confirmDialog, openPagesSection, openViewsSection, waitForConfirmDialog, } from '../fixtures/components';
...
const pagesList = (window) => window.locator('h2:has-text("Pages") + ul');
/** Add a page from the sidebar and switch to it. */
const addPage = async (window, name) => {
...
test.describe('convert a page to a view', () => {
    test('moves the page into views/ and leaves a wrapper at the route', async ({ window, project, }) => {
        ...
        await expect(window.getByTestId('component-editor-banner')).toContainText('Editing view: Home');
