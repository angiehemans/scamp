 ...
 * its bindings from the samples, nothing is rewritten on open, and the
 * banner reports the framework isn't installed.
 * see docs/plans/framework-phase-1-plan.md, step 6
 */
test('the contract fixture opens as a scamp project and is left byte-identical', async ({ window, project, }) => {
...
test('an installed framework inside the supported range shows no banner', async ({ window, project }) => {
    const pkgDir = path.join(project.dir, 'node_modules', 'scampjs');
    await fs.mkdir(pkgDir, { recursive: true });
    await fs.writeFile(path.join(pkgDir, 'package.json'), JSON.stringify({ name: 'scampjs', version: '0.0.5', scampjs: { contract: 0 } }));
