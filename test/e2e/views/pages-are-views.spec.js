...
import { clickContextMenuItem, confirmDialog, createComponentFromSidebar, createPageFromSidebar, openPageContextMenu, openPagesSection, waitForConfirmDialog, } from '../fixtures/components';
...
test.describe('pages are views', () => {
    test('+ Add Page creates a view, its route wrapper, and opens it as a page', async ({ window, project, }) => {
    ...
    test('Esc does not leave a view, and it can be drawn on and saved', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
    ...
    test('deleting a view page removes its folder and its route wrapper', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
    ...
    test('renaming a view page by slug moves the folder and the wrapper', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
