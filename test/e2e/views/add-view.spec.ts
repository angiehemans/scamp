...
test.describe('views sidebar', () => {
  test('creates views/<Name> in the contract shape, a wrapper page, and opens the editor', async ({
    window,
    project,
  }) => {
    ...
    await expect(
      window.getByRole('button', { name: 'hero-card', exact: true })
    ).toHaveCount(0);
  });
  ...
  test('a view is not offered as a draggable instance', async ({ window }) => {
    ...
    await expect(viewSidebarItem(window, 'Landing')).toHaveAttribute(
      'draggable',
      'false'
    );
  });
  ...
  test('deleting a view removes its folder and its wrapper page', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
