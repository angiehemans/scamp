 ...
 */
test('a new view opens on a page-sized artboard that the size control agrees with', async ({ window, }) => {
    await expect(pageRoot(window)).toBeVisible();
    await createPageFromSidebar(window, 'landing');
    await expect(window.getByTitle('Select page root')).toHaveText('landing');
    await expect(window.getByRole('button', { name: /1440 × 900/ })).toBeVisible();
