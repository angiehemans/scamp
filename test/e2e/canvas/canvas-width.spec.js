...
import { canvasSizeButton, canvasSizePopover, switchBreakpoint, } from '../fixtures/breakpoints';
...
test.describe('canvas: custom width input', () => {
    test('typing a custom canvas width drops the breakpoint back to Desktop', async ({ window, }) => {
        ...
        await expect(canvasSizeButton(window)).toHaveAttribute('data-active-breakpoint', 'tablet');
        ...
        await expect(canvasSizeButton(window)).toHaveAttribute('data-active-breakpoint', 'desktop');
    });
});
