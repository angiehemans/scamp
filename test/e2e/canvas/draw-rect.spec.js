...
test.describe('canvas: draw rectangle', () => {
    test('R + drag creates a rect on the canvas and on disk', async ({ window, project, }) => {
    ...
    test('R + click drops a default-sized rect centered on the cursor', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await selectTool(window, 'r');
