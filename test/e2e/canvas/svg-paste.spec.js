...
import { canvasElementsByPrefix, canvasFrame, pageRoot, tokenSwatch, } from '../fixtures/selectors';
...
test.describe('canvas: paste SVG from clipboard', () => {
    ...
    test('pastes a shape as a real, valid-JSX svg child (renders on the canvas)', async ({ window, app, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await app.evaluate(({ clipboard }) => {
    ...
    test('is labeled "SVG" in the layers tree and is click-selectable on the canvas', async ({ window, app, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await app.evaluate(({ clipboard }) => {
            clipboard.writeText('<svg viewBox="0 0 100 100" width="100" height="100"><rect width="100" height="100" fill="#ff0000"/></svg>');
        });
    ...
    test('an outline icon: invisible box dropped, strokes recolor via Stroke', async ({ window, app, project, }) => {
        ...
        await app.evaluate(({ clipboard }) => {
            clipboard.writeText('<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="#111111" stroke-width="2"><path d="M0 0h24v24H0z" fill="none" stroke="none"/><path d="M4 6h16"/></svg>');
        });
        await canvasFrame(window).click({ position: { x: 4, y: 4 } });
        await window.keyboard.press('ControlOrMeta+v');
        await waitForSaved(window);
    ...
    test('recolors via a theme TOKEN (resolved on the canvas)', async ({ window, app, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await app.evaluate(({ clipboard }) => {
            clipboard.writeText('<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="#111111" stroke-width="2"><path d="M4 6h16"/></svg>');
        });
        await canvasFrame(window).click({ position: { x: 4, y: 4 } });
        await window.keyboard.press('ControlOrMeta+v');
