...
const drawRect = async (
  window: Parameters<typeof dragInFrame>[0],
...
test.describe('canvas: select / move / resize', () => {
  test('clicking a rect selects it; clicking empty canvas deselects', async ({
    window,
  }) => {
  ...
  test('drag-moves an element and writes the new position to disk', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('SE resize handle grows the rect and writes the new size', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
