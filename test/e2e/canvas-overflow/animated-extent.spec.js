...
const appliedScale = async (page) => canvasFrame(page).evaluate((frame) => {
    const el = frame;
    const width = el.getBoundingClientRect().width;
    return el.offsetWidth > 0 ? Number((width / el.offsetWidth).toFixed(3)) : 1;
});
test.describe('canvas extent: animated elements', () => {
    test('an element that only overflows mid-animation never reports overflow', async ({ window, }) => {
    ...
    test('the zoom holds steady across edits at different animation phases', async ({ window, }) => {
        await expect(canvasElement(window, 'rect_drift')).toBeVisible();
