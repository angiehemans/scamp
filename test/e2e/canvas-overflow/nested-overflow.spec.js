...
const markerFor = (page, id) => page.locator(`[data-element-overflow="${id}"]`);
test.describe('nested container overflow', () => {
    ...
    test('stays silent for a container with room to spare', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await expect(markerFor(window, 'a001')).toBeVisible();
        await expect(markerFor(window, 'a004')).toHaveCount(0);
    });
    test('does not flicker for a child that only leaves mid-animation', async ({ window, }) => {
    ...
    test('is canvas chrome, so it never bakes into an export', async ({ window, }) => {
        await expect(markerFor(window, 'a001')).toBeVisible();
        const chromeAncestor = await markerFor(window, 'a001').evaluate((el) => el.closest('[data-canvas-chrome="true"]') !== null);
        expect(chromeAncestor).toBe(true);
        // And it must not intercept canvas clicks.
        const clickable = await markerFor(window, 'a001').evaluate((el) => globalThis.getComputedStyle(el).pointerEvents);
    ...
    test('markers sit over the container they describe', async ({ window }) => {
        ...
        if (boxes === null)
            throw new Error('missing marker or target');
