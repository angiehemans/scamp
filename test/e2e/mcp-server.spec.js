...
test.describe('MCP server, live', () => {
    ...
    test('answers about an imported page from main, not from the canvas', async ({ window, project, }) => {
    ...
    test('describes a view’s props from a file an agent just wrote, by name or slug', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const config = await readConfig(project.dir);
        ...
        await fs.writeFile(join(dir, 'Lobby.module.css'), ['.root {', '  width: 100%;', '  position: relative;', '}', '', '.code_e1d2 {', '  width: 200px;', '}', '', '.row_e1e4 {', '  width: 200px;', '}', '', '.note_e1f5 {', '  width: 200px;', '}', '', '.start_e1f9 {', '  width: 200px;', '}', ''].join('\n'), 'utf-8');
        await fs.writeFile(join(dir, 'Lobby.tsx'), [
