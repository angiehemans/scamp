...
test.describe('grouping: Cmd+G / Cmd+Shift+G', () => {
  test('Cmd+G wraps selected siblings in a flex group with display:flex', async ({
    window,
    project,
  }) => {
    ...
    expect(css).not.toMatch(
      new RegExp(`\\.${firstClass}\\s*\\{[^}]*left:\\s*80px`)
    );
  });
