...
test.describe('grouping: nested groups', () => {
    test('grouping an existing group produces a group-of-group', async ({ window, }) => {
        ...
        // secondClass, or G1.
        const allClasses = await canvasElementsByPrefix(window, 'rect_').evaluateAll((els) => els.map((el) => el.dataset.scampId ?? ''));
        const otherClasses = allClasses.filter((c) => c !== firstClass && c !== secondClass);
