...
const dragOverRow = async (
  ...
  await page.evaluate(
    ({ sourceSel, targetSel, frac, mime }) => {
      const panel = '[data-testid="layers-panel"]';
      const src = document.querySelector(
        `${panel} [data-element-class="${sourceSel}"]`
      ) as HTMLElement | null;
      const dst = document.querySelector(
        `${panel} [data-element-class="${targetSel}"]`
      ) as HTMLElement | null;
...
async function drawRects(page: Page, count: number): Promise<string[]> {
  ...
  const classes = await canvasElementsByPrefix(page, 'rect_').evaluateAll((nodes) =>
...
const dropLine = (page: Page, className: string) =>
  layersRowByClass(page, className).locator('div[class*="dropLine"]');
...
test.describe('layers panel: drop placement feedback', () => {
  ...
  test('the top band of the same row offers "beside" instead', async ({
    window,
  }) => {
    const [first, second] = await drawRects(window, 2);
    ...
    await expect(layersRowByClass(window, first!)).not.toHaveClass(
      /rowDropInside/
    );
  ...
  test('an image row never offers "inside"', async ({ window, app, project }) => {
    ...
    if (!imgClass) throw new Error('no image element on the canvas');

    // Dead centre of the image row — the most "inside" position there is.
    await dragOverRow(window, rect!, imgClass, 0.5);

    await expect(layersRowByClass(window, imgClass)).not.toHaveClass(
      /rowDropInside/
    );
    await expect(dropLine(window, imgClass)).toHaveCount(1);
  });

  test('the insertion line is indented to the depth it drops at', async ({
    window,
  }) => {
    ...
    if (!inner) throw new Error('expected a nested rect');
    ...
    const nestedLeft = await dropLine(window, inner).evaluate(
      (el) => (el as HTMLElement).style.left
    );

    await dragOverRow(window, inner, outer!, 0.05);
    const outerLeft = await dropLine(window, outer!).evaluate(
      (el) => (el as HTMLElement).style.left
    );
