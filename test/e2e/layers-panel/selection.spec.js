...
import { canvasElementsByPrefix, pageRoot, resizeHandle, } from '../fixtures/selectors';
import { waitForSaved } from '../fixtures/assertions';
test.describe('layers panel: selection', () => {
    test('clicking a row selects the canvas element (resize handles appear)', async ({ window, }) => {
    ...
    test('layer rows appear in DFS order (Page first, then children)', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await selectTool(window, 'r');
