...
test.describe('layers panel: hover tooltip shows the CSS class name', () => {
    test('hovering a row surfaces `.{className}` via the tooltip portal', async ({ window, }) => {
        await expect(pageRoot(window)).toBeVisible();
        const cls = await drawAndSelectRect(window, { x: 100, y: 100 }, { x: 240, y: 200 });
        ...
        await expect(window.getByRole('tooltip').filter({ hasText: `.${cls}` })).toBeVisible({ timeout: 2_000 });
    });
});
