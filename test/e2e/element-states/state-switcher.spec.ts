...
test.describe('element states: state switcher', () => {
  test('the switcher renders four buttons when an element is selected', async ({
    window,
  }) => {
  ...
  test('editing in Hover writes a :hover rule block', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}:hover\\s*\\{[^}]*opacity:\\s*0\\.5`,
        's'
      )
    );
  });

  test('editing in Active and Focus writes their respective blocks', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(
        `\\.${className}:active\\s*\\{[^}]*opacity:\\s*0\\.8`,
        's'
      )
    );
    expect(css).toMatch(
      new RegExp(
        `\\.${className}:focus\\s*\\{[^}]*opacity:\\s*0\\.9`,
        's'
      )
    );
  });

  test('the state-dot indicator appears on Hover after an override is added', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await drawAndSelectRect(
    ...
    // not present.
    await expect(
      hoverButton.locator('[aria-label="has overrides"]')
    ).toHaveCount(0);
  ...
  test('Default state edits the base, leaving any state blocks intact', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(css).toMatch(
      new RegExp(`\\.${className}\\s*\\{[^}]*width:\\s*320px`, 's')
    );
  ...
  test('right-click override dot resets the state-axis override', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    const hoverBlock = css.match(
      new RegExp(`\\.${className}:hover\\s*\\{[^}]*\\}`, 's')
    );
    expect(hoverBlock).toBeNull();
  });

  test('non-desktop breakpoints disable the non-default state buttons', async ({
    window,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    await drawAndSelectRect(
    ...
    const switchBreakpoint = await import('../fixtures/breakpoints').then(
      (m) => m.switchBreakpoint
    );
