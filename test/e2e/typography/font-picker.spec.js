...
test.describe('typography: font picker', () => {
    test('picking a font commits a CSS font-family value', async ({ window, project, }) => {
        ...
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*font-family:[^;]+;`, 's'));
    });
    test('custom-font escape hatch commits a typed name when no enumerated font matches', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        await selectTool(window, 't');
        await clickInFrame(window, { x: 180, y: 180 });
        await window.keyboard.press('Escape');
        ...
        await expect(window.getByRole('option', { name: /^Use "FakeFontDoesNotExist 9000"/ })).toBeVisible();
        await search.press('Enter');
        await waitForSaved(window);
        const { css } = await readPageFiles(project.dir, project.pageName);
        expect(css).toMatch(new RegExp(`\\.${className}[^}]*font-family:[^;]*FakeFontDoesNotExist 9000`, 's'));
    });
});
