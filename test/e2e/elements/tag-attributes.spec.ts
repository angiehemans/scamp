...
test.describe('elements: tag-specific attributes', () => {
  ...
  test('dialog: Open checkbox emits the boolean attribute', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
    ...
    expect(tsx).toMatch(
      new RegExp(
        `<dialog[^>]*data-scamp-id="${className}"[^>]*\\sopen(\\s|/?>)`,
        's'
      )
    );
  });

  test('form: method GET / POST picks up the value attribute', async ({
    window,
    project,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
