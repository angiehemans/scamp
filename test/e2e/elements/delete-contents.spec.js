...
import { clickContextMenuItem, openElementContextMenu, } from '../fixtures/components';
...
test.describe('elements: delete contents', () => {
    test('removes the children but keeps the parent element', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        // A parent rect with a child drawn inside it.
        const parentClass = await drawAndSelectRect(window, { x: 60, y: 60 }, { x: 400, y: 300 });
        await waitForSaved(window);
        const childClass = await drawAndSelectRect(window, { x: 90, y: 90 }, { x: 180, y: 160 });
        await waitForSaved(window);
