...
test.describe('elements: tag change', () => {
    test('changing a rect to <nav> writes <nav> in the TSX; class prefix stays rect_', async ({ window, project, }) => {
    ...
    test('changing a text element to <h1> writes <h1>', async ({ window, project, }) => {
        await expect(pageRoot(window)).toBeVisible();
        ...
        if (!className)
            throw new Error('text element missing class');
