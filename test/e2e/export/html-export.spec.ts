...
const stubOpenPath = async (
  app: Parameters<typeof stubOpenDialog>[0]
): Promise<void> => {
...
test.describe('export: HTML', () => {
  test('writes a browsable folder of HTML and CSS for the project', async ({
    window,
    app,
    project,
  }) => {
  ...
  test('creates its own folder inside the one you pick, leaving it untouched', async ({
    window,
    app,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
  ...
  test('exports beside the previous one rather than over an unrelated folder', async ({
    window,
    app,
  }) => {
    await expect(pageRoot(window)).toBeVisible();
