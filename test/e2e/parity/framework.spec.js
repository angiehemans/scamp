...
import { compareGeometry, describeDivergences, launchTruthBrowser, measure, measureUrlInBrowser, startFrameworkServer, } from './harness';
/**
...
const writeProject = async (dir, fixture, themeCss) => {
    const files = {
...
const instanceSubtree = (canvas, instanceId) => {
...
for (const fixture of PARITY_FIXTURES) {
    ...
    test.describe(`framework parity: ${fixture.name}`, () => {
        ...
        test(`canvas matches scamp dev${fixture.knownGap ? ' (known gap)' : ''}`, async ({ window, project, }) => {
            ...
            try {
                const mine = plan.kind === 'instance' ? instanceSubtree(canvas, plan.instanceId) : canvas;
                const view = plan.kind === 'instance' ? plan.component : 'Home';
                const width = plan.kind === 'instance' ? (mine['root']?.w ?? rootWidth ?? 0) : (rootWidth ?? 0);
