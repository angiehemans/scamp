...
import { compareGeometry, describeDivergences, launchTruthBrowser, measureInBrowser, } from './harness';
/**
 ...
 */
const renderAndMeasure = async (html, css, themeCss, width) => (await windowRef.evaluate(async ([htmlIn, cssIn, themeIn, widthIn]) => {
...
test.describe('iframe spike: does an in-app iframe match a plain browser', () => {
    ...
    test('a media query follows the artboard width, which is the whole point', async ({ window, project, }) => {
...
`;
        ...
        expect(await widthAt(390), 'mobile artboard applies the media query').toBe(120);
    });
});
