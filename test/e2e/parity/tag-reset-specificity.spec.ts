...
const computed = async (
  ...
  canvasElement(window, className).evaluate(
    (el, p) => globalThis.getComputedStyle(el).getPropertyValue(p),
    prop
  );

test.describe('canvas tag reset: the page wins, the reset only fills in', () => {
  test('an anchor keeps the colour and type its own CSS sets', async ({
    window,
  }) => {
    ...
    expect(await computed(window, 'styled_link', 'color')).toBe(
      'rgb(0, 200, 100)'
    );
  ...
  test('a button keeps the colour and size its own CSS sets', async ({
    window,
  }) => {
    await expect(canvasElement(window, 'styled_button')).toBeVisible();
    expect(await computed(window, 'styled_button', 'color')).toBe(
      'rgb(200, 0, 100)'
    );
    expect(await computed(window, 'styled_button', 'font-size')).toBe('27px');
  });

  test('the reset still strips browser chrome the page says nothing about', async ({
    window,
  }) => {
    ...
    expect(
      await computed(window, 'bare_anchor', 'text-decoration-line')
    ).toBe('none');
  });

  test('an untagged text element was never affected either way', async ({
    window,
  }) => {
