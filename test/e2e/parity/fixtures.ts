...
export const PARITY_FIXTURES: ReadonlyArray<ParityFixture> = [
  {
    ...
    knownGap:
      "Scamp's model defaults `alignItems` to `flex-start`, but CSS defaults it to `stretch`. The generator only emits `align-items` when it differs from the model default, so the file says nothing and the browser stretches; the canvas applies `flex-start` unconditionally and the children stay content-height. Affects every flex container that never set align-items.",
...
`,
  },
  {
    ...
    knownGap:
      'The canvas grid branch reads `columnGap` / `rowGap` but never `el.gap`, so a container using the `gap` shorthand gets no gap on the canvas. The flex branch beside it does use `el.gap`.',
    tsx: page(`    <div data-scamp-id="root" className={styles.root}>
