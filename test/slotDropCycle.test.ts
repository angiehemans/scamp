...
import type { ReparentDrop } from '@renderer/src/canvas/interactions/types';
...
const absoluteDrop = (targetId: string, slotName?: string): ReparentDrop => ({
  kind: 'absolute',
  targetId,
...
describe('slotDropCreatesCycle', () => {
  ...
  const pageElements: Record<string, ScampElement> = {
  ...
  it('allows dropping an instance into a slot while on a page (no active component)', () => {
    expect(
      slotDropCreatesCycle(
        absoluteDrop('inst_card', 'children'),
        'inst_banner',
        pageElements,
        componentTrees,
        null
  ...
  it('ignores drops whose target is not a component-instance', () => {
    const withRect: Record<string, ScampElement> = {
      ...pageElements,
      plain_rect: makeEl({ id: 'plain_rect' }),
    };
    expect(
      slotDropCreatesCycle(
        absoluteDrop('plain_rect'),
        'inst_banner',
        withRect,
        componentTrees,
        'Card'
