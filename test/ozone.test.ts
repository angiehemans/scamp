...
describe('resolveOzonePlatform', () => {
  describe('non-Linux platforms', () => {
    it('appends nothing on macOS', () => {
      expect(
        resolveOzonePlatform({ platform: 'darwin', argv: [], override: undefined }),
      ).toBe(null);
    });

    it('appends nothing on Windows', () => {
      expect(
        resolveOzonePlatform({ platform: 'win32', argv: [], override: undefined }),
      ).toBe(null);
    });

    it('appends nothing on macOS even when the override is set', () => {
      expect(
        resolveOzonePlatform({ platform: 'darwin', argv: [], override: 'wayland' }),
      ).toBe(null);
    });
  });
  ...
  describe('the Linux default', () => {
    it('forces x11 when nothing is specified', () => {
      expect(
        resolveOzonePlatform({ platform: LINUX, argv: [], override: undefined }),
      ).toBe('x11');
    });

    it('forces x11 when the override is an empty string', () => {
      expect(
        resolveOzonePlatform({ platform: LINUX, argv: [], override: '' }),
      ).toBe('x11');
    });
  ...
  describe('the SCAMP_OZONE_PLATFORM override', () => {
    it('appends nothing for "auto" so Electron picks the backend', () => {
      expect(
        resolveOzonePlatform({ platform: LINUX, argv: [], override: 'auto' }),
      ).toBe(null);
    });

    it('passes "wayland" through so users can opt back in', () => {
      expect(
        resolveOzonePlatform({ platform: LINUX, argv: [], override: 'wayland' }),
      ).toBe('wayland');
    });
