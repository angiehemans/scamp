...
describe('rewriteComponentForRename', () => {
    it('updates the function name + CSS module import basename', () => {
        ...
        const renamed = rewriteComponentForRename(original.tsx, original.css, 'Button', 'PrimaryButton');
        expect(renamed.tsx).toContain("import styles from './PrimaryButton.module.css';");
    ...
    it('preserves the element tree and default values after rename', () => {
        ...
        const renamed = rewriteComponentForRename(original.tsx, original.css, 'Button', 'NewName');
        const reparsed = parseCode(renamed.tsx, renamed.css);
        // The text element survives + its default value is intact.
        const propTexts = Object.values(reparsed.elements).filter((el) => el.type === 'text' && typeof el.prop === 'string');
...
describe('rewritePageForComponentRename', () => {
    const buildPage = (componentNames) => {
        const elements = {
    ...
    it('returns changed: false when the page does not reference oldName', () => {
        ...
        const out = rewritePageForComponentRename(page.tsx, page.css, 'Button', 'PrimaryButton', 'home', 'legacy');
    ...
    it('rewrites the import + JSX tag when the page does reference oldName', () => {
        ...
        expect(out.tsx).not.toContain("import Button from '@/components/Button/Button';");
    });
