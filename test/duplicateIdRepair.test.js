...
const shortOf = (cls) => cls.includes('_') ? cls.slice(cls.lastIndexOf('_') + 1) : cls;
describe('parseCode — duplicate id repair', () => {
    ...
    it('is deterministic — the same file repairs to the same ids', () => {
        ...
        expect(Object.keys(a.elements).sort()).toEqual(Object.keys(b.elements).sort());
    });
    ...
    it('round-trips: after generate the repaired file has no more duplicates', () => {
        ...
        expect(Object.keys(second.elements).sort()).toEqual(Object.keys(first.elements).sort());
    });
});
