...
import {
  parseTpgid,
  shellBaseName,
} from '../src/main/ipc/terminalForeground';

describe('parseTpgid', () => {
  it('parses a normal /proc/PID/stat line', () => {
    // Real example from bash, abbreviated. Field 8 (tpgid) is 1234.
    const stat =
      '5678 (bash) S 1 5678 5678 34816 1234 4194304 100 200 0 0 0 0 0 0 20 0 1 0 100 12345 6789';
  ...
  it('handles comm fields with spaces and parentheses', () => {
    ...
    // is the right anchor.
    const stat =
      '5678 (the (real) name) S 1 5678 5678 34816 7777 4194304 100 200 0 0 0 0 0 0 20 0 1 0 100 12345 6789';
  ...
  it('returns null when tpgid is zero or negative', () => {
    const zero =
      '5678 (bash) S 1 5678 5678 34816 0 4194304 100 200';
    expect(parseTpgid(zero)).toBeNull();
    const negative =
      '5678 (bash) S 1 5678 5678 34816 -1 4194304 100 200';
