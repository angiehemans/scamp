...
import { hasProjectThumbnail, MAX_THUMBNAIL_BYTES, readProjectThumbnail, thumbnailPathFor, writeProjectThumbnail, } from '../src/main/ipc/projectThumbnailOps';
/**
...
describe('readProjectThumbnail', () => {
    ...
    it('returns null for a project folder that is gone', async () => {
        expect(await readProjectThumbnail({ projectPath: path.join(dir, 'missing') })).toEqual({ base64: null });
    });
    it('round-trips what was written', async () => {
        await writeProjectThumbnail({ projectPath: dir, dataUrl: PNG_DATA_URL });
        const { base64 } = await readProjectThumbnail({ projectPath: dir });
        if (base64 === null)
            throw new Error('expected a thumbnail');
        const rewritten = await writeProjectThumbnail({
            projectPath: dir,
            dataUrl: `data:image/png;base64,${base64}`,
        });
        expect(rewritten.ok).toBe(true);
        expect((await readProjectThumbnail({ projectPath: dir })).base64).toBe(base64);
    });
});
