...
const NEXT_ISMS: ReadonlyArray<[string, RegExp]> = [
...
describe('AGENT_MD_CONTENT_SCAMP', () => {
  ...
  it('mentions `app/` only to say there isn\'t one', () => {
    ...
    for (const line of lines) expect(line).toMatch(/There is no/);
  });
  ...
  it('says how a data-driven image binds, which is where a catalogue lands', () => {
    const assets = AGENT_MD_CONTENT_SCAMP.slice(
      AGENT_MD_CONTENT_SCAMP.indexOf('## Images and static assets')
    );
