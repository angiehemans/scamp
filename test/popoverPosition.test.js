...
describe('computePopoverPosition', () => {
    it('places the popover below the trigger by default', () => {
        const pos = computePopoverPosition(rect({ top: 100, bottom: 120, left: 50, right: 150, width: 100 }), { width: 200, desiredMaxHeight: 300, align: 'left' }, VIEWPORT);
    ...
    it('clamps maxHeight to the available space below', () => {
        const pos = computePopoverPosition(rect({ top: 600, bottom: 620 }), { width: 200, desiredMaxHeight: 400, align: 'left' }, VIEWPORT);
        // spaceBelow = 800 - 620 - edgeMargin(8) - gap(4) = 168
        expect(pos.placedAbove).toBe(false);
        expect(pos.maxHeight).toBe(168);
    });
    it('flips above when there is not enough room below', () => {
        const pos = computePopoverPosition(rect({ top: 700, bottom: 720, left: 50, right: 150, width: 100 }), { width: 200, desiredMaxHeight: 300, align: 'left' }, VIEWPORT);
    ...
    it('keeps the popover within the left/right viewport edges', () => {
        const pos = computePopoverPosition(rect({ top: 100, bottom: 120, left: 950, right: 990, width: 40 }), { width: 200, desiredMaxHeight: 300, align: 'left' }, VIEWPORT);
    ...
    it('does not overlay when a side fits, even with overlayWhenTight', () => {
        ...
        const pos = computePopoverPosition(rect({ top: 100, bottom: 120, height: 20 }), {
            width: 200,
            desiredMaxHeight: 420,
            align: 'left',
            minFitBelow: 420,
    ...
    it('aligns the popover to the trigger right edge when align=right', () => {
        const pos = computePopoverPosition(rect({ top: 100, bottom: 120, left: 300, right: 400, width: 100 }), { width: 200, desiredMaxHeight: 300, align: 'right' }, VIEWPORT);
