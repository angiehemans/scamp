...
vi.mock('../src/renderer/src/lib/exportCapture', () => ({
  captureIsolatedPng: (args: unknown) => {
...
import {
  ...
  flushPendingProjectThumbnail,
} from '../src/renderer/src/lib/projectThumbnail';
...
beforeEach(() => {
  ...
  const frame = document.querySelector('[data-testid="canvas-frame"]') as HTMLElement;
  ...
  (globalThis as unknown as { window: { scamp: unknown } }).window.scamp = {
    writeProjectThumbnail: ({ projectPath }: { projectPath: string }) => {
...
const scheduleFor = (projectPath: string): void => {
  useCanvasStore.setState({ projectPath } as never);
...
describe('a scheduled project thumbnail', () => {
  it('does not photograph a project that replaced the one it was scheduled for', async () => {
    ...
    useCanvasStore.setState({ projectPath: '/tmp/beta' } as never);
  ...
  it('does not photograph anything once every project is closed', async () => {
    scheduleFor('/tmp/alpha');
    useCanvasStore.setState({ projectPath: null } as never);
    await vi.advanceTimersByTimeAsync(5000);
    expect(captured.calls).toEqual([]);
  });
