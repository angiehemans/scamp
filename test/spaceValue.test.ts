...
import {
  ...
  spaceTupleEquals,
  spaceValueEquals,
  spaceValueNumberOrZero,
  tokenSpaceValue,
  type SpaceTuple,
  type SpaceValue,
} from '@lib/spaceValue';

const tok = (ref: string): SpaceValue => tokenSpaceValue(ref);
...
describe('SpaceValue helpers', () => {
  describe('formatSpaceValue', () => {
    ...
    it('emits the token ref verbatim', () => {
      ...
      expect(formatSpaceValue(tok('var(--gutter, 16px)'))).toBe(
        'var(--gutter, 16px)'
      );
    });
  });
  ...
  describe('spaceValueEquals', () => {
    ...
    it('compares tokens by ref string', () => {
      expect(
        spaceValueEquals(tok('var(--space-md)'), tok('var(--space-md)'))
      ).toBe(true);
      expect(
        spaceValueEquals(tok('var(--space-md)'), tok('var(--space-sm)'))
      ).toBe(false);
    });
  ...
  describe('spaceTupleEquals', () => {
    ...
    it('handles mixed token / number tuples', () => {
      const a: SpaceTuple = [16, tok('var(--space-md)'), 16, tok('var(--space-md)')];
      const b: SpaceTuple = [16, tok('var(--space-md)'), 16, tok('var(--space-md)')];
  ...
  describe('formatSpaceShorthand', () => {
    ...
    it('emits four values when fully asymmetric', () => {
      expect(formatSpaceShorthand([16, 24, 32, 8])).toBe(
        '16px 24px 32px 8px'
      );
    });
    ...
    it('keeps two distinct tokens separate (no collapse when they differ)', () => {
      ...
      const tm = tok('var(--space-md)');
      expect(formatSpaceShorthand([ts, tm, ts, tm])).toBe(
