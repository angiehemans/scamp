...
const cssFor = (elements) => generateCode({ elements, rootId: ROOT_ELEMENT_ID, pageName: 'home' }).css;
const block = (css, cls) => {
...
describe('flex container emission', () => {
    ...
    it('emits nothing for the defaults', () => {
        const b = block(cssFor({ [ROOT_ELEMENT_ID]: makeRoot(['a1b2']), a1b2: makeRect({ id: 'a1b2', display: 'flex' }) }), 'rect_a1b2');
...
describe('breakpoint / state override lines', () => {
    ...
    it('emits the item fields, with an empty basis clearing to auto', () => {
        const lines = breakpointOverrideLines({ flexGrow: 1, flexShrink: 0, flexBasis: '', order: 1 }, el, flexParent);
        expect(lines).toEqual(expect.arrayContaining(['flex-grow: 1;', 'flex-shrink: 0;', 'flex-basis: auto;', 'order: 1;']));
    });
