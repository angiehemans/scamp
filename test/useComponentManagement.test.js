...
const stubScampApi = () => {
    const api = {
        ...
        createComponent: vi.fn(async (args) => {
...
const harness = (pages) => {
    const setActivePageName = vi.fn((next) => {
        calls.push(`setActivePageName:${String(next)}`);
    });
    const rendered = renderHook(() => useComponentManagement({
...
describe('convertAllPagesToViews', () => {
    ...
    it('moves the canvas off the open page before it converts anything', async () => {
        ...
        expect(calls.indexOf('setActivePageName:null')).toBeLessThan(calls.indexOf('createComponent:Home'));
    });
