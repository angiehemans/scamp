...
import {
  ...
  startHeartbeat,
  type HeartbeatDeps,
} from '../src/main/auth/heartbeat';
import type { FetchLike } from '../src/main/auth/tokenExchange';

/**
 * The heartbeat is how the product learns anyone is using the app, so the
 * cases that matter are the ones where it should stay silent: signed out,
 * offline, or already sent. And the one response that must act: a 401,
 * where the app has to stop claiming the user is signed in.
 */
...
describe('sendHeartbeat', () => {
  ...
  it('signs the user out on a 401', async () => {
    ...
    expect(await sendHeartbeat(deps({ fetchImpl: responder(401) }))).toBe(
      'unauthorized'
    );
  ...
  it('does NOT sign the user out on a server error', async () => {
    expect(await sendHeartbeat(deps({ fetchImpl: responder(500) }))).toBe(
      'failed'
    );
    expect(unauthorizedCount).toBe(0);
  });
  ...
  it('reports failure rather than throwing when the server is down', async () => {
    ...
    await expect(sendHeartbeat(deps({ fetchImpl: down }))).resolves.toBe(
      'failed'
    );
  });
