...
import { assemblePageCss, collectKeyframeNames, prefixCss, prefixSelectorClasses, rewriteCssUrls, } from '@lib/htmlExportCss';
describe('prefixSelectorClasses', () => {
    ...
    it('prefixes every class in a selector list', () => {
        expect(prefixSelectorClasses('.root, .label_d3e4', 'p__')).toBe('.p__root, .p__label_d3e4');
    });
    ...
    it('prefixes classes in compound and pseudo selectors', () => {
        expect(prefixSelectorClasses('.btn.primary:hover', 'p__')).toBe('.p__btn.p__primary:hover');
    });
    ...
    it('prefixes a class that sits beside an attribute selector', () => {
        expect(prefixSelectorClasses('.row[data-open]', 'p__')).toBe('.p__row[data-open]');
    });
...
describe('prefixCss', () => {
    ...
    it('prefixes rules nested inside a media query, leaving the prelude alone', () => {
        const out = prefixCss('@media (max-width: 700px) {\n  .root { display: none; }\n}', 'p__');
    ...
    it('renames keyframes so two stylesheets cannot claim the same animation', () => {
        const out = prefixCss('@keyframes fade { from { opacity: 0; } to { opacity: 1; } }', 'p__');
    ...
    it('rewrites animation references to match the renamed keyframes', () => {
        const out = prefixCss('.spinner { animation: fade 2s ease-in-out infinite; }\n' +
            '@keyframes fade { from { opacity: 0; } }', 'p__');
    ...
    it('rewrites an animation reference inside a media query', () => {
        ...
        const out = prefixCss('@keyframes fade { from { opacity: 0; } }\n' +
            '@media (min-width: 600px) { .spinner { animation-name: fade; } }', 'p__');
...
describe('collectKeyframeNames', () => {
    it('finds every declared animation name', () => {
        const names = collectKeyframeNames('@keyframes fade { } @keyframes slide-in { }');
...
describe('rewriteCssUrls', () => {
    it('rewrites a bare url', () => {
        expect(rewriteCssUrls('.a { background: url(/assets/x.webp); }', () => '../assets/x.webp')).toContain('url(../assets/x.webp)');
    });
...
describe('assemblePageCss', () => {
    ...
    it('returns just the page CSS when there are no instances', () => {
        expect(assemblePageCss('.root { color: red; }', []).trim()).toBe('.root { color: red; }');
    });
});
