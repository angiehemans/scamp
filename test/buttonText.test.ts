...
describe('parseCode — <button> text', () => {
  it('parses a plain <button> label as a text element with tag button', () => {
    const tsx = wrap(
      `<button data-scamp-id="cta1" className={styles.cta_cta1}>Sign up</button>`
    );
  ...
  it('honours a text_ prefix on a button label', () => {
    const tsx = wrap(
      `<button data-scamp-id="cta1" className={styles.text_cta1}>Save</button>`
    );
  ...
  it('upgrades a button with element children to a rectangle container', () => {
    const tsx = wrap(
      `<button data-scamp-id="cta1" className={styles.cta_cta1}>` +
  ...
  it('keeps a rect_-prefixed button as a container (explicit prefix wins)', () => {
    const tsx = wrap(
      `<button data-scamp-id="b1" className={styles.rect_b1}>Sign up</button>`
    );
...
describe('generateCode — <button> text', () => {
  it('emits a text button as <button …>label</button>', () => {
    const tsx = wrap(
      `<button data-scamp-id="cta1" className={styles.text_cta1}>Sign up</button>`
    );
