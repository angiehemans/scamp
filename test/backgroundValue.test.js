...
import { classifyBackgroundValue, isImageBackground, } from '@lib/backgroundValue';
/**
...
describe('classifyBackgroundValue — images', () => {
    it('routes a radial gradient to background-image', () => {
        ...
        expect(classifyBackgroundValue('radial-gradient(circle 640px at 50% 410px, rgba(160,140,255,.2) 0%, rgba(7,6,15,0) 70%)')).toBe('image');
    });
    it('routes a linear gradient to background-image', () => {
        expect(classifyBackgroundValue('linear-gradient(to right, #fff, #000)')).toBe('image');
    });
    it('routes a conic gradient to background-image', () => {
        expect(classifyBackgroundValue('conic-gradient(from 0deg, red, blue)')).toBe('image');
    });
    it('routes a repeating gradient to background-image', () => {
        expect(classifyBackgroundValue('repeating-linear-gradient(45deg, #000 0 10px, #fff 10px 20px)')).toBe('image');
    });
    ...
    it('routes multiple stacked gradients to background-image', () => {
        expect(classifyBackgroundValue('linear-gradient(#0000, #000), radial-gradient(circle, #fff, #0000)')).toBe('image');
    });
    it('is case-insensitive about the function name', () => {
        expect(classifyBackgroundValue('LINEAR-GRADIENT(to top, red, blue)')).toBe('image');
    });
});
describe('classifyBackgroundValue — positioned shorthands', () => {
    it('keeps a value with a position/size slash on the shorthand', () => {
        ...
        expect(classifyBackgroundValue('url(/a.png) center / cover')).toBe('shorthand');
    });
    ...
    it('does not mistake a gradient stop percentage for a shorthand', () => {
        expect(classifyBackgroundValue('linear-gradient(to right, red 0%, blue 100%)')).toBe('image');
    });
});
