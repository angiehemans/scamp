...
import { elementEdits, tsxSurgicalEdits, type EditSide } from '@lib/tsxElementEdits';
...
const GENERATED = (text: string): string => `import styles from './home.module.css';
...
const side = (text: string): EditSide => {
...
describe('elementEdits', () => {
  ...
  it('edits the opening tag when an attribute changed', () => {
    const next = GENERATED('Hello').replace(
      '<div data-scamp-id="rect_b2c2" className={styles.rect_b2c2} />',
      '<div data-scamp-id="rect_b2c2" className={styles.rect_b2c2} title="hi" />'
    );
...
describe('tsxSurgicalEdits', () => {
  ...
  it('carries a change outside the component alongside the element edit', () => {
    const next = GENERATED('Goodbye').replace(
