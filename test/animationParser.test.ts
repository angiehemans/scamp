...
import type { ElementAnimation } from '@lib/element';
...
describe('parseAnimationShorthand', () => {
  ...
  it('parses the full canonical shorthand', () => {
    expect(
      parseAnimationShorthand('fade-in-up 300ms ease forwards')
    ).toEqual({
      ...baseDefaults,
      name: 'fade-in-up',
      isPreset: true,
  ...
  it('parses every fill-mode value', () => {
    for (const fill of ['none', 'forwards', 'backwards', 'both'] as const) {
  ...
  it('parses cubic-bezier easing without splitting on the inner commas', () => {
    const parsed = parseAnimationShorthand(
      'fade-in-up 300ms cubic-bezier(0.4, 0, 0.2, 1) forwards'
    );
  ...
  it('handles tokens in any order between time/easing groups', () => {
    ...
    const parsed = parseAnimationShorthand(
      'shake 500ms ease forwards alternate paused 2'
    );
  ...
  it('returns null on multi-animation source (top-level commas)', () => {
    ...
    expect(
      parseAnimationShorthand('fade-in 300ms, slide-in-left 500ms')
    ).toBeNull();
  });
  ...
  it('does NOT split inside cubic-bezier when checking for multi-animation', () => {
    ...
    const parsed = parseAnimationShorthand(
      'fade-in-up 300ms cubic-bezier(0.4, 0, 0.2, 1)'
    );
