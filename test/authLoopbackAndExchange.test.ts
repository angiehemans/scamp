...
import {
  ...
  type FetchLike,
} from '../src/main/auth/tokenExchange';
...
const start = async (port = PORT, timeoutMs?: number) => {
  ...
  if (result.status === 'listening') cleanup.push(result.close);
  return result;
};
...
describe('exchanging the code for a token', () => {
  ...
  it('returns the token and the user', async () => {
    expect(
      await exchangeCodeForToken({ ...args, fetchImpl: ok({ token: 't', user }) })
    ).toEqual({ status: 'ok', token: 't', user });
  });
  ...
  it('rejects a response with no token', async () => {
    expect(
      await exchangeCodeForToken({ ...args, fetchImpl: ok({ user }) })
