...
describe('parsePayload — single messages', () => {
  ...
  it('keeps params intact', () => {
    const out = parsePayload(
      '{"jsonrpc":"2.0","id":"a","method":"tools/call","params":{"name":"x"}}'
    );
    if (out.kind !== 'single') throw new Error('expected single');
...
describe('parsePayload — malformed input', () => {
  ...
  it('rejects an object id, which JSON-RPC does not permit', () => {
    expect(
      parsePayload('{"jsonrpc":"2.0","id":{"a":1},"method":"ping"}')
...
describe('parsePayload — batches', () => {
  it('parses several messages', () => {
    ...
    if (out.kind !== 'batch') throw new Error('expected batch');
  ...
  it('rejects the whole batch when one entry is malformed', () => {
    ...
    expect(
      parsePayload('[{"jsonrpc":"2.0","id":1,"method":"ping"},{"bad":true}]')
    ).toMatchObject({ kind: 'error' });
  });
});
