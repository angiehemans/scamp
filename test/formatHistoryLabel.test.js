...
describe('formatHistoryLabel — action kinds', () => {
    it('renders "Drew rectangle" for draw-rect', () => {
        ...
        expect(formatHistoryLabel(entry({ kind: 'draw-rect' }), elements)).toBe('Drew rectangle');
    });
    it('renders "Added text" for add-text', () => {
        expect(formatHistoryLabel(entry({ kind: 'add-text' }), {})).toBe('Added text');
    });
    ...
    it('renders "Resized rect_a1b2"', () => {
        const elements = { a1b2: makeRect('a1b2') };
        expect(formatHistoryLabel(entry({ kind: 'resize', elementIds: ['a1b2'] }), elements)).toBe('Resized rect_a1b2');
    });
    it('renders "Changed background — rect_a1b2" for a patch entry', () => {
        const elements = { a1b2: makeRect('a1b2') };
        expect(formatHistoryLabel(entry({
            kind: 'patch',
            elementIds: ['a1b2'],
            propertyKeys: ['backgroundColor'],
        }), elements)).toBe('Changed background — rect_a1b2');
    });
    it('dedupes parallel property keys (width width)', () => {
        ...
        }), elements)).toBe('Changed width — rect_a1b2');
    });
    it('joins distinct property keys with comma', () => {
        ...
        expect(formatHistoryLabel(entry({
            ...
            elementIds: ['a1b2'],
            propertyKeys: ['x', 'y'],
        }), elements)).toBe('Changed left, top — rect_a1b2');
    });
    it('falls back to "styles" when no property keys are given', () => {
        const elements = { a1b2: makeRect('a1b2') };
        expect(formatHistoryLabel(entry({ kind: 'patch', elementIds: ['a1b2'] }), elements)).toBe('Changed styles — rect_a1b2');
    });
    it('renders "Edited styles — rect_a1b2" for raw-css', () => {
        const elements = { a1b2: makeRect('a1b2') };
        expect(formatHistoryLabel(entry({ kind: 'raw-css', elementIds: ['a1b2'] }), elements)).toBe('Edited styles — rect_a1b2');
    });
    it('renders "Renamed old to new"', () => {
        ...
        }), elements)).toBe('Renamed rect_a1b2 to hero_card_a1b2');
    });
    it('renders "Added page home"', () => {
        expect(formatHistoryLabel(entry({ kind: 'add-page', pageName: 'home' }), {})).toBe('Added page home');
    });
    ...
    it('renders "External edit detected"', () => {
        expect(formatHistoryLabel(entry({ kind: 'external-edit' }), {})).toBe('External edit detected');
    });
});
describe('formatHistoryLabel — retroactive rename', () => {
    it('uses the CURRENT element name even on old entries', () => {
        ...
        const elements = { a1b2: makeRect('a1b2', { name: 'hero card' }) };
        expect(formatHistoryLabel(e, elements)).toBe('Changed background — hero_card_a1b2');
    });
