...
const buildCss = (element, rootChildIds = [element.id]) => {
    const elements = {
...
describe('blend mode: generator', () => {
    ...
    it('emits background-blend-mode for a non-default value', () => {
        const css = buildCss(makeRect('a1b2', { backgroundBlendMode: 'overlay' }));
...
describe('blend mode: round-trip', () => {
    ...
    it('round-trips a tablet-breakpoint background-blend-mode', () => {
        ...
        if (!tabletId)
            throw new Error('expected default tablet breakpoint');
        const original = makeRect('a1b2', {
            backgroundBlendMode: 'normal',
            breakpointOverrides: {
        ...
        expect(parsed['a1b2']?.breakpointOverrides?.[tabletId]?.backgroundBlendMode).toBe('multiply');
    });
});
