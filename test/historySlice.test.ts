...
// ScampElement shape.
const snapshot = (tag: string): Record<string, ScampElement> => ({
  [tag]: { __tag: tag } as unknown as ScampElement,
});
...
describe('historySlice — basic stack', () => {
  ...
  it('identity-equality short-circuits a no-op commit', () => {
    ...
    commitHistory({ kind: 'draw-rect' }, snap);
    expect(useHistoryStore.getState().perPage['home.tsx']?.entries).toHaveLength(
      1
    );
  });
...
describe('historySlice — coalescing', () => {
  ...
  it('does NOT coalesce when property keys differ', () => {
    ...
    const { commitHistory } = useHistoryStore.getState();
    commitHistory(
      { kind: 'patch', elementIds: ['a1b2'], propertyKeys: ['backgroundColor'] },
      snapshot('s1')
    );
    commitHistory(
      { kind: 'patch', elementIds: ['a1b2'], propertyKeys: ['opacity'] },
      snapshot('s2')
    );
    expect(
      useHistoryStore.getState().perPage['home.tsx']?.entries
    ).toHaveLength(2);
  });
  ...
  it('does NOT coalesce when element ids differ', () => {
    ...
    commitHistory(
      { kind: 'patch', elementIds: ['c3d4'], propertyKeys: ['backgroundColor'] },
      snapshot('s2')
    );
    expect(
      useHistoryStore.getState().perPage['home.tsx']?.entries
  ...
  it('does NOT coalesce across kinds (move + patch)', () => {
    ...
    const { commitHistory } = useHistoryStore.getState();
    commitHistory(
      { kind: 'move', elementIds: ['a1b2'] },
      snapshot('s1')
    );
    commitHistory(
      { kind: 'patch', elementIds: ['a1b2'], propertyKeys: ['x'] },
      snapshot('s2')
    );
    expect(
      useHistoryStore.getState().perPage['home.tsx']?.entries
...
describe('historySlice — transactions', () => {
  ...
  it('skips commits during a transaction and records one entry on end', () => {
    useHistoryStore.getState().setActivePageId('home.tsx');
    const { beginHistoryTransaction, commitHistory, endHistoryTransaction } =
...
describe('historySlice — per-page isolation', () => {
  ...
  it('keeps separate stacks per page id', () => {
    ...
    expect(Object.keys(state.perPage['home.tsx']!.entries[0]!.snapshot)[0]).toBe(
      'home1'
    );
  });
  ...
  it('rekeyPage moves a page history under a new key (page rename)', () => {
    const { setActivePageId, commitHistory, rekeyPage } =
      useHistoryStore.getState();
    setActivePageId('home.tsx');
    commitHistory({ kind: 'draw-rect' }, snapshot('home1'));
...
describe('historySlice — external edits', () => {
  ...
  it('an external edit trims forward history beyond the current cursor', () => {
    const { setActivePageId, commitHistory, undo, enqueueExternalEdit } =
      useHistoryStore.getState();
    setActivePageId('home.tsx');
    commitHistory({ kind: 'draw-rect' }, snapshot('s1'));
    commitHistory({ kind: 'draw-rect' }, snapshot('s2'));
  ...
  it('an external edit during a transaction is deferred and drained on transaction end', () => {
    ...
    expect(
      useHistoryStore.getState().pendingExternalEdit?.snapshot
    ).toEqual(snapshot('ext1'));
    expect(
      useHistoryStore.getState().perPage['home.tsx']?.entries ?? []
    ).toHaveLength(0);
