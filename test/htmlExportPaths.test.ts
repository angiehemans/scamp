...
describe('page paths', () => {
  ...
  it('co-locates a page stylesheet with its markup', () => {
    ...
    // sideways for it.
    expect(cssPathFor('about').replace('index.css', '')).toBe(
      htmlPathFor('about').replace('index.html', '')
    );
  });
});
...
describe('rewriteAssetUrlForPage', () => {
  it('turns a server-root asset path into a relative one for the home page', () => {
    expect(rewriteAssetUrlForPage('/assets/hero.webp', 'home')).toBe(
      'assets/hero.webp'
    );
  });

  it('climbs out of a page directory for a nested page', () => {
    expect(rewriteAssetUrlForPage('/assets/hero.webp', 'about')).toBe(
      '../assets/hero.webp'
    );
  });
  ...
  it('leaves an absolute http url alone', () => {
    expect(rewriteAssetUrlForPage('https://cdn.example.com/x.png', 'about')).toBe(
      'https://cdn.example.com/x.png'
    );
  });

  it('leaves a data url alone', () => {
    expect(rewriteAssetUrlForPage('data:image/svg+xml;base64,AAA', 'about')).toBe(
      'data:image/svg+xml;base64,AAA'
    );
  });

  it('leaves a protocol-relative url alone', () => {
    expect(rewriteAssetUrlForPage('//cdn.example.com/x.png', 'about')).toBe(
...
describe('rewriteHrefForPage', () => {
  ...
  it('links sideways between two nested pages', () => {
    expect(rewriteHrefForPage('/contact', 'about', pages)).toBe(
      '../contact/index.html'
    );
  });

  it('leaves an external url alone', () => {
    expect(rewriteHrefForPage('https://example.com', 'home', pages)).toBe(
      'https://example.com'
    );
  });
