...
import { sizeTypeOf, sizeTypeLabel, sizeTypeHasNumber, rawForType, combineTypedWithType, } from '@lib/sizeType';
describe('sizeTypeOf', () => {
...
describe('combineTypedWithType', () => {
    ...
    it('respects a full CSS length or keyword typed verbatim', () => {
        ...
        expect(combineTypedWithType('calc(100% - 8px)', 'percent')).toBe('calc(100% - 8px)');
    });
    ...
    it('accepts every type in rawForType without throwing', () => {
        ...
        for (const t of types)
            expect(typeof rawForType(t, 10)).toBe('string');
    });
});
