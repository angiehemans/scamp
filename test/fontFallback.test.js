...
import { formatFontValue, inferFallback, quoteFamilyName, } from '@renderer/lib/fontFallback';
...
describe('quoteFamilyName', () => {
    ...
    it('quotes names with punctuation', () => {
        expect(quoteFamilyName('Source Code Pro, 500')).toBe('"Source Code Pro, 500"');
    });
...
describe('formatFontValue', () => {
    it('combines quoted family + inferred fallback', () => {
        expect(formatFontValue('Helvetica Neue')).toBe('"Helvetica Neue", sans-serif');
        expect(formatFontValue('Fira Code')).toBe('"Fira Code", monospace');
        expect(formatFontValue('Playfair Display')).toBe('"Playfair Display", serif');
    });
