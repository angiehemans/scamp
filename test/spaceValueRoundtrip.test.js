...
import { parsePaddingShorthandOrNull, parseSpaceValueOrNull, parseVarTokenOrNull, } from '@lib/parsers';
...
describe('parsers — var() token support', () => {
    describe('parseVarTokenOrNull', () => {
        ...
        it('parses var() with a fallback verbatim', () => {
            expect(parseVarTokenOrNull('var(--space-md, 16px)')).toBe('var(--space-md, 16px)');
        });
    ...
    describe('parsePaddingShorthandOrNull with tokens', () => {
        ...
        it('parses 4-value form mixing tokens and pixels', () => {
            const result = parsePaddingShorthandOrNull('var(--space-sm) 16px var(--space-md) 24px');
            expect(result).toEqual([
...
`;
    ...
    it('mixed px+var in padding round-trips with the original ordering', () => {
        ...
        expect(first.css).toContain('padding: var(--space-sm) 16px var(--space-md) 24px;');
    ...
    it('var() in gap, column-gap, row-gap round-trip', () => {
        ...
        const parsed = parseCode(first.tsx, first.css);
        expect(parsed.elements['a1b2'].gap).toEqual(tokenSpaceValue('var(--space-md)'));
    });
