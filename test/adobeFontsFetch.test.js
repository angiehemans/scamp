...
import { extractFontFamilies, fetchAdobeKitFamilies, } from '@lib/adobeFontsFetch';
...
describe('fetchAdobeKitFamilies', () => {
    ...
    it('returns families from a 200 response', async () => {
        fetch.mockResolvedValueOnce({
            ok: true,
            status: 200,
            text: async () => '@font-face { font-family: "source-sans-pro"; src: url(""); }\n' +
    ...
    it('returns a typed error when the response has no @font-face blocks', async () => {
        ...
        expect(result.ok).toBe(false);
        if (!result.ok)
            expect(result.error).toContain('@font-face');
    });
});
