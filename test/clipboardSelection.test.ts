...
const normalize = (
  ids: string[],
  elements = tree()
): string[] => normalizeCopySelection(elements, ids, ROOT_ELEMENT_ID);
...
describe('normalizeCopySelection: malformed trees', () => {
  it('terminates when the parent chain loops', () => {
    ...
    expect(() =>
      normalizeCopySelection(elements, ['a', 'b'], ROOT_ELEMENT_ID)
    ).not.toThrow();
  });
  ...
  it('emits each element once when the tree references one twice', () => {
    ...
    expect(normalizeCopySelection(elements, [ROOT_ELEMENT_ID], ROOT_ELEMENT_ID)).toEqual(
      ['outer']
    );
  });
});
