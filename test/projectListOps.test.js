...
import { attachCardMeta, mergeProjectsForDisplay, } from '../src/main/ipc/projectListOps';
const recent = (path, lastOpened, over = {}) => ({
...
describe('mergeProjectsForDisplay', () => {
    ...
    it('overlays the recents timestamp onto a matching scanned project', () => {
        const result = mergeProjectsForDisplay([recent('/projects/alpha', '2026-06-01T00:00:00.000Z')], [scanned('/projects/alpha')]);
    ...
    it('includes recents that the scan did not surface (opened from elsewhere)', () => {
        const result = mergeProjectsForDisplay([recent('/elsewhere/beta', '2026-06-02T00:00:00.000Z')], [scanned('/projects/alpha')]);
    ...
    it('sorts never-opened projects alphabetically by name', () => {
        const result = mergeProjectsForDisplay([], [scanned('/p/delta'), scanned('/p/bravo'), scanned('/p/charlie')]);
        expect(result.map((p) => p.name)).toEqual(['bravo', 'charlie', 'delta']);
    });
    it('dedupes by path, preferring the freshly-scanned format', () => {
        const result = mergeProjectsForDisplay([recent('/projects/alpha', '2026-06-01T00:00:00.000Z', { format: 'legacy' })], [scanned('/projects/alpha', { format: 'nextjs' })]);
        expect(result).toHaveLength(1);
        expect(result[0]?.format).toBe('nextjs');
    });
    it('carries a stale recent (missing folder) through with exists=false', () => {
        const result = mergeProjectsForDisplay([recent('/gone/ghost', '2026-06-03T00:00:00.000Z', { exists: false })], []);
        expect(result[0]).toMatchObject({ path: '/gone/ghost', exists: false });
    });
});
const startProject = (over = {}) => ({
    name: 'Proj',
    path: '/p/proj',
    format: 'nextjs',
    lastOpened: null,
