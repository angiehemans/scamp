...
`;
        ...
        expect(el?.customSelectorBlocks?.[0]?.selector).toBe('.rect_a1b2:focus-visible');
        expect(el?.customSelectorBlocks?.[0]?.body).toContain('outline: 2px solid #0080ff');
    });
...
`;
        ...
        expect(el?.customSelectorBlocks?.[0]?.selector).toBe('.rect_a1b2:hover .child');
    });
...
`;
        const parsed = parseCode(HOME_TSX, css);
        const el = parsed.elements['a1b2'];
        expect(el?.customSelectorBlocks?.[0]?.selector).toBe('.rect_a1b2:nth-child(odd)');
    });
...
`;
        ...
        expect(second.elements['a1b2']?.customSelectorBlocks?.[0]?.selector).toBe('.rect_a1b2:focus-visible');
    });
});
