...
import type { FilterDef } from '@lib/element';
...
describe('parseFilterList', () => {
  ...
  it('parses a multi-filter list space-separated', () => {
    expect(
      parseFilterList('blur(4px) brightness(120%) grayscale(20%)')
    ).toEqual([
      { kind: 'blur', value: 4 },
...
describe('formatFilterList', () => {
  ...
  it('emits a single filter with its canonical unit', () => {
    ...
    expect(formatFilterList([{ kind: 'brightness', value: 120 }])).toBe(
      'brightness(120%)'
    );
    expect(formatFilterList([{ kind: 'hue-rotate', value: 90 }])).toBe(
      'hue-rotate(90deg)'
    );
  });
  ...
  it('omits trailing-zero decimals', () => {
    ...
    expect(formatFilterList([{ kind: 'brightness', value: 120.5 }])).toBe(
      'brightness(120.5%)'
    );
  });
...
describe('parseFilter round-trip', () => {
  const ROUND_TRIP_CASES: ReadonlyArray<FilterDef> = [
    { kind: 'blur', value: 4 },
    { kind: 'brightness', value: 120 },
