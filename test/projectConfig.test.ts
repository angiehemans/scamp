...
describe('parseProjectConfig — componentCanvas', () => {
  it('returns undefined componentCanvas when the field is absent', () => {
    const config = parseProjectConfig(
      JSON.stringify({ canvasWidth: 800 })
    );
  ...
  it('round-trips through serialize → parse', () => {
    ...
    expect(parseProjectConfig(serializeProjectConfig(config))).toEqual(
      config
    );
  });
});
...
describe('parseProjectConfig — card meta', () => {
  it('parses a valid card background color', () => {
    const cfg = parseProjectConfig(
      JSON.stringify({ cardBackground: '#3b82f6' })
    );
  ...
  it('drops an invalid card background (non-string / control chars)', () => {
    expect(
      parseProjectConfig(JSON.stringify({ cardBackground: 42 })).cardBackground
    ).toBeUndefined();
    expect(
      parseProjectConfig(JSON.stringify({ cardBackground: 'red\n' }))
        .cardBackground
    ).toBeUndefined();
  });
  ...
  it('trims and length-caps the state string', () => {
    const cfg = parseProjectConfig(
      JSON.stringify({ state: '  ready to review  ' })
    );
  ...
  it('round-trips card meta through serialize → parse', () => {
    const written = serializeProjectConfig(
      parseProjectConfig(
        JSON.stringify({ cardBackground: '#112233', state: 'in progress' })
      )
    );
