...
const seed = (extra: Record<string, ScampElement>, samples?: ScampElement['samples']): void => {
...
const el = (id: string): ScampElement => {
  const e = useCanvasStore.getState().elements[id];
  if (!e) throw new Error(`missing ${id}`);
  return e;
};
const historyLength = (): number =>
  useHistoryStore.getState().perPage[HISTORY_ID]?.entries.length ?? 0;
