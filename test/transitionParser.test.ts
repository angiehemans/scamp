...
describe('splitCssList', () => {
  ...
  it('preserves commas inside parens', () => {
    expect(
      splitCssList('opacity 200ms ease, transform 300ms cubic-bezier(0.4, 0, 0.2, 1)')
    ).toEqual([
      'opacity 200ms ease',
...
describe('parseTransitionSegment', () => {
  ...
  it('preserves cubic-bezier expressions verbatim', () => {
    expect(
      parseTransitionSegment('opacity 200ms cubic-bezier(0.4, 0, 0.2, 1)')
    ).toEqual({
      property: 'opacity',
      durationMs: 200,
  ...
  it('lowercases named easings but preserves cubic-bezier casing', () => {
    expect(parseTransitionSegment('opacity 200ms EASE-IN-OUT')?.easing).toBe(
      'ease-in-out'
    );
    expect(
      parseTransitionSegment('opacity 200ms CUBIC-BEZIER(0.1, 0.2, 0.3, 0.4)')
        ?.easing
...
describe('parseTransitionShorthand', () => {
  ...
  it('parses comma-separated transitions', () => {
    expect(
      parseTransitionShorthand(
        'opacity 200ms ease, transform 300ms ease-in-out 100ms'
      )
    ).toEqual([
      { property: 'opacity', durationMs: 200, easing: 'ease', delayMs: 0 },
      {
  ...
  it('handles cubic-bezier without splitting on its inner commas', () => {
    expect(
      parseTransitionShorthand(
        'opacity 200ms ease, transform 300ms cubic-bezier(0.4, 0, 0.2, 1) 50ms'
      )
    ).toEqual([
      { property: 'opacity', durationMs: 200, easing: 'ease', delayMs: 0 },
      {
...
describe('formatTransitionShorthand', () => {
  ...
  it('emits the delay segment when non-zero', () => {
    ...
    expect(formatTransitionShorthand([t])).toBe(
  ...
  it('joins multiple transitions with commas', () => {
    ...
    expect(formatTransitionShorthand(ts)).toBe(
      'opacity 200ms ease, transform 300ms linear'
    );
  });
});
