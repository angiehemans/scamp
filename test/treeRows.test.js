...
const idsOf = (rows) => rows.map((r) => (r.kind === 'element' ? r.element.id : `raw:${r.parentId}`));
describe('flattenTree', () => {
    ...
    it('hides a collapsed element descendants but keeps the element', () => {
        expect(idsOf(flattenTree(tree(), ROOT_ELEMENT_ID, { outer: true }))).toEqual([ROOT_ELEMENT_ID, 'outer', 'sibling']);
    });
    ...
    it('does not shift depths when a branch collapses', () => {
        ...
        const depthOf = (rows, id) => rows.find((r) => r.kind === 'element' && r.element.id === id)?.depth;
    ...
    it('hides the raw row when its element is collapsed', () => {
        ...
        expect(idsOf(flattenTree(elements, ROOT_ELEMENT_ID, { outer: true }))).toEqual([ROOT_ELEMENT_ID, 'outer', 'sibling']);
    });
...
describe('hasChildRows', () => {
    ...
    it('is true for an element with only inline fragments', () => {
        ...
        expect(hasChildRows(el({ id: 'x', inlineFragments: [fragment('hi')] }))).toBe(true);
    });
