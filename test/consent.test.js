...
describe('hasCurrentConsent', () => {
    ...
    it('is false for a decision made against older wording', () => {
        ...
        expect(hasCurrentConsent(settings({ sentryOptIn: true, consentVersion: 0 }))).toBe(false);
    });
    it('is true once answered against the current wording', () => {
        expect(hasCurrentConsent(settings({ sentryOptIn: true, consentVersion: CONSENT_VERSION }))).toBe(true);
    });
    it('counts an explicit NO as answered', () => {
        // A decline still resolves the prompt — we mustn't nag.
        expect(hasCurrentConsent(settings({ sentryOptIn: false, consentVersion: CONSENT_VERSION }))).toBe(true);
    });
    it('accepts a future version, so a downgrade does not re-prompt', () => {
        expect(hasCurrentConsent(settings({ sentryOptIn: true, consentVersion: CONSENT_VERSION + 1 }))).toBe(true);
    });
});
describe('isOptedIn', () => {
    it('is false for a stale opt-in', () => {
        // The guarantee: nothing is sent under an old agreement.
        expect(isOptedIn(settings({ sentryOptIn: true, consentVersion: 0 }))).toBe(false);
    });
    it('is true for a current opt-in', () => {
        expect(isOptedIn(settings({ sentryOptIn: true, consentVersion: CONSENT_VERSION }))).toBe(true);
    });
    it('is false for a current opt-out', () => {
        expect(isOptedIn(settings({ sentryOptIn: false, consentVersion: CONSENT_VERSION }))).toBe(false);
    });
    it('is false on a fresh install', () => {
...
describe('parseSettingsBlob — consent fields', () => {
    ...
    it('round-trips a current consent', () => {
        const parsed = parseSettingsBlob(JSON.stringify({ sentryOptIn: true, consentVersion: CONSENT_VERSION }));
        expect(isOptedIn(parsed)).toBe(true);
    });
    it('reads a stored install id', () => {
        const id = 'f81d4fae-7dec-41d0-9765-00a0c91e6bf6';
        expect(parseSettingsBlob(JSON.stringify({ installId: id })).installId).toBe(id);
    });
