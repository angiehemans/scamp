...
describe('wouldCreateComponentCycle', () => {
  ...
  const tree = (
    name: string,
    nestedComponentNames: ReadonlyArray<string> = []
  ): { elements: Record<string, ScampElement>; rootId: string } => {
    const elements: Record<string, ScampElement> = {
      [ROOT_ELEMENT_ID]: makePageRoot(
        nestedComponentNames.map((_, i) => `n${name}_${i}`)
      ),
    };
  ...
  it('refuses dropping a component into its own editor (direct cycle)', () => {
    ...
    expect(wouldCreateComponentCycle(componentTrees, 'Button', 'Button')).toBe(
      true
    );
  });
  ...
  it('returns false when targetName is null (page editor — no cycle possible)', () => {
    const componentTrees = { Button: tree('Button') };
    expect(wouldCreateComponentCycle(componentTrees, null, 'Button')).toBe(
      false
    );
  });
