...
import {
  ...
  THUMBNAIL_MAX_WIDTH,
  thumbnailFrame,
} from '../src/renderer/lib/thumbnailCrop';
...
describe('thumbnailFrame', () => {
  ...
  it('returns null for NaN dimensions', () => {
    expect(
      thumbnailFrame({ sourceWidth: Number.NaN, sourceHeight: 900 })
    ).toBeNull();
    expect(
      thumbnailFrame({ sourceWidth: 1280, sourceHeight: Number.NaN })
    ).toBeNull();
  });

  it('returns null for an unusable aspect or width', () => {
    expect(
      thumbnailFrame({ sourceWidth: 1280, sourceHeight: 900, aspect: 0 })
    ).toBeNull();
    expect(
      thumbnailFrame({ sourceWidth: 1280, sourceHeight: 900, maxWidth: 0 })
    ).toBeNull();
  });
