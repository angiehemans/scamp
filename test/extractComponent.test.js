...
import { extractSubtreeAsComponent, generateComponentFromSubtree, } from '@lib/extractComponent';
...
describe('extractSubtreeAsComponent', () => {
    ...
    it('keeps every descendant in the extracted map', () => {
        ...
        const result = extractSubtreeAsComponent(elements, 'a1b2');
        expect(result).not.toBeNull();
        expect(Object.keys(result.elements).sort()).toEqual([ROOT_ELEMENT_ID, 'c3d4', 'e5f6'].sort());
    });
...
describe('generateComponentFromSubtree', () => {
    ...
    it('returns null when the subtree root is missing', () => {
        ...
        expect(generateComponentFromSubtree(elements, 'a1b2', 'Button')).toBeNull();
    });
