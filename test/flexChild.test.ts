...
const parentWith = (
  display: ScampElement['display'],
  flexDirection: ScampElement['flexDirection'] = 'row'
): ScampElement => element({ id: 'parent', parentId: null, display, flexDirection });
...
describe('shrinkGuardPatch — px on the main axis means don’t shrink', () => {
  ...
  it('treats the reverse directions by axis', () => {
    expect(
      shrinkGuardPatch(element(), parentWith('flex', 'row-reverse'), 'width', 'fixed')
    ).toEqual({ flexShrink: 0 });
    expect(
      shrinkGuardPatch(element(), parentWith('flex', 'column-reverse'), 'height', 'fixed')
    ).toEqual({ flexShrink: 0 });
  });

  it('clears a guard Scamp set when the axis leaves px', () => {
    for (const mode of ['stretch', 'fit-content', 'auto'] as const) {
      expect(
        shrinkGuardPatch(element({ flexShrink: 0 }), parentWith('flex', 'row'), 'width', mode)
      ).toEqual({ flexShrink: 1 });
    }
  });

  it('never touches a hand-set shrink value', () => {
    expect(
      shrinkGuardPatch(element({ flexShrink: 0.5 }), parentWith('flex', 'row'), 'width', 'fixed')
    ).toEqual({});
    expect(
      shrinkGuardPatch(element({ flexShrink: 0.5 }), parentWith('flex', 'row'), 'width', 'stretch')
    ).toEqual({});
  });
...
describe('dropDrawnOffset', () => {
  const drawn = (): ScampElement => ({ ...element(), x: 406, y: 416 });
