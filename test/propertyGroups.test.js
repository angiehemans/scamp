...
describe('FIELD_TO_GROUP', () => {
    it('is the inverse of GROUP_FIELDS', () => {
        for (const [group, fields] of Object.entries(GROUP_FIELDS)) {
            for (const field of fields) {
                expect(FIELD_TO_GROUP[field]).toBe(group);
            }
        }
    });
    it('returns undefined for fields not in any group', () => {
        expect(FIELD_TO_GROUP['widthMode']).toBeUndefined();
        expect(FIELD_TO_GROUP['display']).toBeUndefined();
        expect(FIELD_TO_GROUP['x']).toBeUndefined();
    });
});
...
describe('canonicalizeGroupList', () => {
    ...
    it('dedupes', () => {
        expect(canonicalizeGroupList(['shadow', 'shadow', 'background', 'shadow'])).toEqual(['background', 'shadow']);
    });
