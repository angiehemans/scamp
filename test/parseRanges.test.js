...
`;
const parse = (tsx) => parseCode(tsx, CSS, { breakpoints: [], isComponent: false });
const sliceOf = (tsx, id) => {
    const range = parse(tsx).ranges?.[id];
    if (range === undefined)
        throw new Error(`no range for ${id}`);
    return tsx.slice(range.start, range.end);
};
const innerOf = (tsx, id) => {
    const range = parse(tsx).ranges?.[id];
    if (range === undefined)
        throw new Error(`no range for ${id}`);
...
`;
        expect(sliceOf(tsx, 'a1b1')).toBe('<h1 data-scamp-id="text_a1b1" className={styles.text_a1b1}>Hello</h1>');
        expect(innerOf(tsx, 'a1b1')).toBe('Hello');
        expect(sliceOf(tsx, 'b2c2')).toBe('<div data-scamp-id="rect_b2c2" className={styles.rect_b2c2} />');
...
`;
        expect(sliceOf(tsx, 'a1b1')).toBe('<a data-scamp-id="text_a1b1" className={styles.text_a1b1} href={url}>Go</a>');
...
`;
        expect(sliceOf(tsx, 'b2c2')).toBe('<div data-scamp-id="rect_b2c2" className={styles.rect_b2c2} key={row.id} />');
    });
...
`;
        ...
        expect(tsx.slice(range?.start ?? 0, range?.end ?? 0)).toBe('<h1 data-scamp-id="text_a1b1" className={styles.text_a1b1}>Hi</h1>');
    });
