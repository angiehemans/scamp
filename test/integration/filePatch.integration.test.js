...
describe('file patch integration — media-scoped', () => {
    ...
    it('creates a new @media block when none exists', async () => {
        ...
        await fs.writeFile(cssPath, ORIGINAL, 'utf-8');
        const original = await fs.readFile(cssPath, 'utf-8');
        const next = patchClassBlock(original, 'rect_a1b2', 'padding: 12px 12px 12px 12px;', { maxWidth: 768 });
        await fs.writeFile(cssPath, next, 'utf-8');
        const result = await fs.readFile(cssPath, 'utf-8');
...
`;
        ...
        const original = await fs.readFile(cssPath, 'utf-8');
        const next = patchClassBlock(original, 'rect_a1b2', 'padding: 16px 16px 16px 16px;', { maxWidth: 768 });
        await fs.writeFile(cssPath, next, 'utf-8');
        const result = await fs.readFile(cssPath, 'utf-8');
...
`;
        await fs.writeFile(cssPath, source, 'utf-8');
        const original = await fs.readFile(cssPath, 'utf-8');
        const next = patchClassBlock(original, 'rect_a1b2', 'padding: 12px;', { maxWidth: 768 });
        await fs.writeFile(cssPath, next, 'utf-8');
        const result = await fs.readFile(cssPath, 'utf-8');
    ...
    it('does not touch the base class when patching media-scoped', async () => {
        ...
        await fs.writeFile(cssPath, ORIGINAL, 'utf-8');
        const original = await fs.readFile(cssPath, 'utf-8');
        const next = patchClassBlock(original, 'rect_a1b2', 'padding: 8px;', { maxWidth: 390 });
        const result = next;
...
`;
        await fs.writeFile(cssPath, source, 'utf-8');
        const original = await fs.readFile(cssPath, 'utf-8');
        const next = patchClassBlock(original, 'rect_a1b2', 'padding: 20px;', { maxWidth: 768 });
