...
import {
  copyImage,
  assetsDirFor,
  finishDeferredImport,
} from '../../src/main/ipc/imageOps';
import {
  flatImage,
  noisyImage,
  pngBytes,
  ...
  describe('legacy format', () => {
    it('copies into <project>/assets/ and returns ./assets/<name>', async () => {
      const result = await copyImage(
        { sourcePath, projectPath: projectDir },
        'legacy'
      );
...
describe('copyImage: a file already in the assets folder', () => {
  ...
  const assetNames = async (format: 'legacy' | 'nextjs'): Promise<string[]> =>
    (await fs.readdir(assetsDirFor(projectDir, format))).sort();
  ...
  it('is not fooled by a path that reaches the asset via ..', async () => {
    ...
    const result = await copyImage(
      { sourcePath: indirect, projectPath: projectDir },
      'legacy'
    );
    expect(result.reused).toBe(true);
  ...
  it('still copies a DIFFERENT file that happens to share the name', async () => {
    ...
    const result = await copyImage(
      { sourcePath: other, projectPath: projectDir },
      'legacy'
    );
    expect(result.fileName).toBe('hero-1.png');
  ...
  it('still copies a same-named file from a SUBFOLDER of assets', async () => {
    ...
    const result = await copyImage(
      { sourcePath: nested, projectPath: projectDir },
      'legacy'
    );
    expect(result.fileName).toBe('hero-1.png');
    expect(result.reused).toBe(false);
  ...
  it('matches the filesystem on a case-differing path', async () => {
    ...
    const result = await copyImage(
      { sourcePath: caseInsensitiveFs ? upper : assetPath, projectPath: projectDir },
      'legacy'
    );
    expect(result.reused).toBe(true);
    expect(await assetNames('legacy')).toEqual(['hero.png']);
...
describe('copyImage: deferred conversion', () => {
  ...
  it('copies the original untouched and flags the pending conversion', async () => {
    ...
    const written = await fs.readFile(
      path.join(assetsDirFor(projectDir, 'legacy'), 'hero.png')
    );
  ...
  it('finishes by writing the webp beside the original, leaving it in place', async () => {
    ...
    const done = await finishDeferredImport(
      projectDir,
      copied.fileName,
      'legacy'
    );
...
describe('assetsDirFor', () => {
  it('points to public/assets for nextjs', () => {
    expect(assetsDirFor('/p', 'nextjs')).toBe(
      path.join('/p', 'public', 'assets')
    );
  });
