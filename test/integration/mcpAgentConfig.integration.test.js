...
import { AGENT_TARGETS, ensureGitignoreEntries, isAgentInstalled, mergeServerEntry, SERVER_KEY, writeAgentConfigs, } from '../../src/main/mcp/agentConfig';
/**
...
const content = (result) => {
    if (!result.ok)
        throw new Error(`expected a successful merge: ${result.reason}`);
    return result.content;
};
const parse = (result) => JSON.parse(content(result));
const servers = (config) => {
    const map = config.mcpServers;
    if (map === undefined)
        throw new Error('config has no mcpServers');
    return map;
};
const scampEntry = (config) => {
    const entry = servers(config)[SERVER_KEY];
    if (entry === undefined)
        throw new Error('config has no scamp entry');
    return entry;
};
...
describe('mergeServerEntry — preserving what the user had', () => {
    ...
    it('creates the server map when the file exists without one', () => {
        const merged = parse(mergeServerEntry(JSON.stringify({ theme: 'dark' }), gemini, URL, TOKEN));
        expect(merged['theme']).toBe('dark');
        expect(scampEntry(merged)).toBeDefined();
    });
    it('updates our own entry in place instead of duplicating it', () => {
        const first = content(mergeServerEntry(null, claude, URL, TOKEN));
        const second = parse(mergeServerEntry(first, claude, 'http://127.0.0.1:39850/mcp', 'tok-2'));
...
describe('mergeServerEntry — refusing rather than clobbering', () => {
    ...
    it('refuses when the server map is the wrong type', () => {
        const out = mergeServerEntry(JSON.stringify({ mcpServers: 'nonsense' }), claude, URL, TOKEN);
        expect(out.ok).toBe(false);
    });
...
describe('writeAgentConfigs', () => {
    ...
    const run = () => writeAgentConfigs({ projectPath: project, homeDir: home, url: URL, token: TOKEN });
    const read = async (relative) => fs.readFile(join(project, relative), 'utf-8');
    ...
    it('writes a detected agent config, creating its directory', async () => {
        ...
        expect(JSON.parse(await read('.cursor/mcp.json')).mcpServers[SERVER_KEY].url).toBe(URL);
    });
    ...
    it('preserves an existing config it merges into', async () => {
        await fs.writeFile(join(project, '.mcp.json'), JSON.stringify({ mcpServers: { other: { url: 'https://example/mcp' } } }), 'utf-8');
        await run();
    ...
    it('rewrites the entry when the port changes', async () => {
        ...
        expect(JSON.parse(await read('.mcp.json')).mcpServers[SERVER_KEY].url).toBe('http://127.0.0.1:39845/mcp');
    });
});
