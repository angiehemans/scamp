...
import {
  disposeImportSources,
  listImportSources,
  readImportSource,
  saveImportSource,
  setActiveImportProject,
} from '../../src/main/importSourceStore';
...
const source = (over: Partial<Parameters<typeof saveImportSource>[3]> = {}) => ({
...
describe('saveImportSource', () => {
  ...
  it('records what it could not read, rather than leaving it unexplained', async () => {
    const stored = await saveImportSource(
      project('c'),
      'Lobby',
      'https://x.test/',
      source({ unreadable: ['https://cdn.test/a.css'], truncated: true })
    );
