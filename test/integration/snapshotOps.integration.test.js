...
describe('snapshot operations — nextjs format', () => {
    ...
    it('captures the scaffolded page files into .scamp/snapshots/<id>/', async () => {
        ...
        const liveHome = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
        const snapHome = await fs.readFile(path.join(snapDir, 'app', 'page.tsx'), 'utf-8');
        expect(snapHome).toBe(liveHome);
        const index = JSON.parse(await fs.readFile(path.join(projectDir, '.scamp', 'snapshots.json'), 'utf-8'));
        expect(index.snapshots.map((s) => s.id)).toContain(snap.id);
    });
...
describe('snapshot operations — legacy format', () => {
    ...
    it('captures the flat root page files', async () => {
        ...
        expect(snap).not.toBeNull();
        const snapDir = path.join(projectDir, '.scamp', 'snapshots', snap.id);
        const liveHome = await fs.readFile(path.join(projectDir, 'home.tsx'), 'utf-8');
        const snapHome = await fs.readFile(path.join(snapDir, 'home.tsx'), 'utf-8');
        expect(snapHome).toBe(liveHome);
    });
});
