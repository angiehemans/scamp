...
const page = (width: number): Record<string, ScampElement> => ({
...
const generate = (elements: Record<string, ScampElement>): { tsx: string; css: string } =>
  generateCode({ elements, rootId: ROOT_ELEMENT_ID, pageName: 'home' });
...
describe('merging a refused write', () => {
  ...
  const seed = async (): Promise<{ tsx: string; css: string }> => {
  ...
  it('lands both an agent rewriting the view and a designer resizing a box', async () => {
    ...
    const agentTsx = base.tsx.replace(
