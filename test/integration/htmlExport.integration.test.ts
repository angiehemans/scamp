...
import {
  ...
  writeExportFiles,
  writeMarker,
} from '../../src/main/ipc/htmlExportOps';
...
const projectSources = (): {
  pages: Array<{ name: string; tsxContent: string; cssContent: string }>;
  components: Array<{ name: string; tsxContent: string; cssContent: string }>;
} => ({
  pages: [
    sourceFor('home', [
...
describe('html export on disk', () => {
  ...
  it('copies the project assets', async () => {
    ...
    const copied = await fs.readFile(
      path.join(exportDir, 'assets', 'hero.webp'),
      'utf-8'
    );
  ...
  it('copies assets from nested folders too', async () => {
    ...
    await runExport();
    expect(
      await fs.readFile(path.join(exportDir, 'assets', 'icons', 'star.svg'), 'utf-8')
    ).toBe('<svg/>');
  });
  ...
  it('resolves every local path the markup references', async () => {
    ...
    for (const page of ['index.html', path.join('about', 'index.html')]) {
      ...
      for (const ref of refs) {
        ...
        await expect(
          fs.access(path.resolve(pageDir, ref)),
          `${page} references ${ref}, which does not exist`
  ...
  it('links the two pages to each other', async () => {
    ...
    const about = await fs.readFile(
      path.join(exportDir, 'about', 'index.html'),
      'utf-8'
    );
