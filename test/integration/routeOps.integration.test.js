...
import { listRoutes, parseRenderExport, parseViewImport, readDevVarsKeys, renameRouteForView, renameRouteView, routePathFor, setRenderExport, setRouteRender, writeRouteFile, } from '../../src/main/ipc/routeOps';
...
describe('renameRouteForView', () => {
    ...
    it('moves the route the app named and points it at the new view', async () => {
        ...
        expect(await fs.readFile(path.join(dir, 'routes/contact.tsx'), 'utf-8')).toContain("import Contact from '@/views/Contact/Contact';");
    ...
    it('leaves a route the developer placed where it is, and only fixes its references', async () => {
        ...
        await fs.writeFile(path.join(dir, 'routes', 'company', 'story.tsx'), "import About from '@/views/About/About';\n\nexport default function AboutRoute() {\n  return <About />;\n}\n", 'utf-8');
        const file = await renameRouteForView(dir, 'About', 'Contact');
        // The URL was their decision; the broken import was not.
        expect(file).toBe('company/story.tsx');
        expect(await fs.readFile(path.join(dir, 'routes/company/story.tsx'), 'utf-8')).toContain("import Contact from '@/views/Contact/Contact';");
    });
...
describe('parseDevJsonLine', () => {
    it('turns request and error lines into app-log entries and ignores the rest', () => {
        expect(parseDevJsonLine('{"t":"2026-09-10T15:00:00.000Z","kind":"request","method":"GET","path":"/lobby","status":200,"ms":12}')).toEqual({ level: 'info', message: 'GET /lobby 200 12ms' });
