...
import { createPage, deletePage, duplicatePage, } from '../../src/main/ipc/pageOps';
...
describe('page operations — nextjs format', () => {
    ...
    describe('createPage', () => {
        it('writes app/<name>/page.tsx and app/<name>/page.module.css', async () => {
            const result = await createPage({ projectPath: projectDir, pageName: 'about' }, 'nextjs');
            expect(result.name).toBe('about');
            expect(result.tsxPath).toBe(path.join(projectDir, 'app', 'about', 'page.tsx'));
        ...
        it('PascalCases hyphenated page names in the component', async () => {
            const result = await createPage({ projectPath: projectDir, pageName: 'checkout-flow' }, 'nextjs');
            const tsx = await fs.readFile(result.tsxPath, 'utf-8');
        ...
        it('rejects "home" as a name (the home page is created by the scaffold)', async () => {
            await expect(createPage({ projectPath: projectDir, pageName: 'home' }, 'nextjs')).rejects.toThrow(/already exists/);
        });
        it('rejects when the target folder already exists', async () => {
            await createPage({ projectPath: projectDir, pageName: 'about' }, 'nextjs');
            await expect(createPage({ projectPath: projectDir, pageName: 'about' }, 'nextjs')).rejects.toThrow(/already exists/);
        });
        it('rejects invalid page names', async () => {
            await expect(createPage({ projectPath: projectDir, pageName: 'Not Valid' }, 'nextjs')).rejects.toThrow(/Invalid page name/);
        });
    });
    describe('deletePage', () => {
        it('removes the page folder and its contents', async () => {
            await createPage({ projectPath: projectDir, pageName: 'about' }, 'nextjs');
            await deletePage({ projectPath: projectDir, pageName: 'about' }, 'nextjs');
        ...
        it('refuses to delete the home page', async () => {
            await expect(deletePage({ projectPath: projectDir, pageName: 'home' }, 'nextjs')).rejects.toThrow(/can't be deleted/);
        ...
        it('leaves a non-empty page folder alone (preserves agent leftovers)', async () => {
            ...
            await fs.writeFile(path.join(projectDir, 'app', 'about', 'notes.md'), 'agent leftover', 'utf-8');
            await deletePage({ projectPath: projectDir, pageName: 'about' }, 'nextjs');
            // Folder still exists with the leftover.
            const remaining = await fs.readdir(path.join(projectDir, 'app', 'about'));
    ...
    describe('duplicatePage', () => {
        ...
        it('duplicates the home page into a named folder', async () => {
            ...
            expect(result.tsxPath).toBe(path.join(projectDir, 'app', 'home-copy', 'page.tsx'));
            const tsx = await fs.readFile(result.tsxPath, 'utf-8');
            expect(tsx).toContain('export default function HomeCopy()');
            // Original home page must still exist at app/page.tsx.
            const homeTsx = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
        ...
        it('rejects when the target name already exists', async () => {
            ...
            await expect(duplicatePage({
                ...
                sourcePageName: 'home',
                newPageName: 'about',
            }, 'nextjs')).rejects.toThrow(/already exists/);
        });
    ...
    describe('renamePageFiles', () => {
        it('renames a non-home page folder and rewrites the component name', async () => {
            ...
            expect(result.tsxPath).toBe(path.join(projectDir, 'app', 'company', 'page.tsx'));
            const tsx = await fs.readFile(result.tsxPath, 'utf-8');
        ...
        it('refuses to rename the home page (would break Next.js routing)', async () => {
            await expect(renamePageFiles({
                projectPath: projectDir,
                oldPageName: 'home',
                newPageName: 'landing',
            }, 'nextjs')).rejects.toThrow(/can't be renamed/);
        });
