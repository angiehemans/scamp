...
describe('page rename integration', () => {
    ...
    beforeEach(async () => {
        ...
        await fs.writeFile(path.join(tmpDir, 'home.module.css'), SOURCE_CSS, 'utf-8');
    });
    afterEach(async () => {
    ...
    it('renames both files and rewrites the TSX import + component name', async () => {
        ...
        const newCss = await fs.readFile(path.join(tmpDir, 'landing.module.css'), 'utf-8');
    ...
    it('derives a PascalCase component name from a hyphenated page name', async () => {
        ...
        const newTsx = await fs.readFile(path.join(tmpDir, 'checkout-flow.tsx'), 'utf-8');
    ...
    it('rejects when the new name already exists and leaves old files untouched', async () => {
        ...
        await fs.writeFile(path.join(tmpDir, 'about.module.css'), 'existing', 'utf-8');
        await expect(renamePageFiles({
            projectPath: tmpDir,
            oldPageName: 'home',
            newPageName: 'about',
        }, 'legacy')).rejects.toThrow(/already exists/);
        // Old files must still be present and unchanged.
        expect(await fs.readFile(path.join(tmpDir, 'home.tsx'), 'utf-8')).toBe(SOURCE_TSX);
        expect(await fs.readFile(path.join(tmpDir, 'home.module.css'), 'utf-8')).toBe(SOURCE_CSS);
    });
    it('rejects when the old page is missing', async () => {
        await expect(renamePageFiles({
            ...
            oldPageName: 'missing',
            newPageName: 'landing',
        }, 'legacy')).rejects.toThrow(/Source page/);
    ...
    it('rejects when new === old', async () => {
        await expect(renamePageFiles({
            ...
            oldPageName: 'home',
            newPageName: 'home',
        }, 'legacy')).rejects.toThrow(/same as the old/);
    });
    it('rejects invalid page names', async () => {
        ...
        }, 'legacy')).rejects.toThrow(/Invalid page name/);
    });
    it('rejects when the TSX has no matching CSS-module import', async () => {
        await fs.writeFile(path.join(tmpDir, 'home.tsx'), `export default function Home() { return null; }`, 'utf-8');
        await expect(renamePageFiles({
            projectPath: tmpDir,
            oldPageName: 'home',
            newPageName: 'landing',
        }, 'legacy')).rejects.toThrow(/CSS-module import/);
    ...
    it('rejects when the default-export function signature does not match', async () => {
        await fs.writeFile(path.join(tmpDir, 'home.tsx'), `import styles from './home.module.css';\nconst Home = () => null;\nexport default Home;\n`, 'utf-8');
        await expect(renamePageFiles({
            projectPath: tmpDir,
            oldPageName: 'home',
            newPageName: 'landing',
        }, 'legacy')).rejects.toThrow(/default-export function/);
        const entries = await fs.readdir(tmpDir);
        expect(entries).not.toContain('landing.tsx');
        expect(entries).not.toContain('landing.module.css');
    });
