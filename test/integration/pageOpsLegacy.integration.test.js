...
describe('page operations — legacy format', () => {
    ...
    it('createPage writes <name>.tsx and <name>.module.css at the project root', async () => {
        const result = await createPage({ projectPath: projectDir, pageName: 'about' }, 'legacy');
    ...
    it('createPage allows "home" as a name in legacy (legacy has no special-case)', async () => {
        ...
        await expect(createPage({ projectPath: projectDir, pageName: 'home' }, 'legacy')).rejects.toThrow(/already exists/);
    });
    it('deletePage removes both files', async () => {
        await createPage({ projectPath: projectDir, pageName: 'about' }, 'legacy');
        await deletePage({ projectPath: projectDir, pageName: 'about' }, 'legacy');
    ...
    it('deletePage allows deleting any page in legacy (no home special-case)', async () => {
        ...
        await deletePage({ projectPath: projectDir, pageName: 'home' }, 'legacy');
        const entries = await fs.readdir(projectDir);
