...
describe('page rename refactors href references in sibling pages', () => {
    ...
    beforeEach(async () => {
        ...
        await fs.writeFile(path.join(appDir, 'page.tsx'), HOME_PAGE_WITH_LINKS_TSX, 'utf-8');
        await fs.writeFile(path.join(appDir, 'page.module.css'), HOME_PAGE_CSS, 'utf-8');
        // About page (the one we're renaming).
        const aboutDir = path.join(appDir, 'about');
        await fs.mkdir(aboutDir);
        await fs.writeFile(path.join(aboutDir, 'page.tsx'), ABOUT_PAGE_TSX, 'utf-8');
        await fs.writeFile(path.join(aboutDir, 'page.module.css'), ABOUT_PAGE_CSS, 'utf-8');
    });
    afterEach(async () => {
    ...
    it('rewrites every href reference to the renamed page on sibling pages', async () => {
        ...
        const homeAfter = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
    ...
    it('does not touch sibling pages that have no matching href', async () => {
        ...
        await fs.writeFile(path.join(projectDir, 'app', 'page.tsx'), `import styles from './page.module.css';

export default function Home() {
  return <div className={styles.root} data-scamp-id="root">Hi</div>;
}
`, 'utf-8');
        const before = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
        await renamePageFiles({
            projectPath: projectDir,
            oldPageName: 'about',
            newPageName: 'company',
        }, 'nextjs');
        const after = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
