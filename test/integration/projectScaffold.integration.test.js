...
import { readProjectLegacy, readProjectNextjs, refreshAgentMdIfNeeded, scaffoldLegacyProject, scaffoldNextjsProject, themePathFor, } from '../../src/main/ipc/projectScaffold';
import { AGENT_MD_CONTENT, AGENT_MD_CONTENT_LEGACY, CLAUDE_MD_CONTENT, } from '../../src/shared/agentMd';
...
import { ROOT_ELEMENT_ID } from '../../src/renderer/lib/element';
...
describe('scaffoldNextjsProject', () => {
    ...
    it('writes the default --font-sans token into theme.css', async () => {
        await scaffoldNextjsProject(projectDir, 'my-project');
        const theme = await fs.readFile(path.join(projectDir, 'app', 'theme.css'), 'utf-8');
    ...
    it('readProjectNextjs picks up the home page', async () => {
        ...
        expect(pages[0]?.tsxPath).toBe(path.join(projectDir, 'app', 'page.tsx'));
    });
    it('readProjectNextjs picks up additional pages in app/<name>/', async () => {
        ...
        await fs.writeFile(path.join(aboutDir, 'page.tsx'), `import styles from './page.module.css';\nexport default function About() {\n  return <div data-scamp-id="root" className={styles.root} />;\n}\n`, 'utf-8');
        await fs.writeFile(path.join(aboutDir, 'page.module.css'), '.root {}\n', 'utf-8');
        const pages = await readProjectNextjs(projectDir);
    ...
    it('round-trips through generateCode → file → parseCode for the home page', async () => {
        ...
        await fs.writeFile(path.join(projectDir, 'app', 'page.tsx'), code.tsx, 'utf-8');
        await fs.writeFile(path.join(projectDir, 'app', 'page.module.css'), code.css, 'utf-8');
        const tsx = await fs.readFile(path.join(projectDir, 'app', 'page.tsx'), 'utf-8');
...
describe('scaffoldLegacyProject', () => {
    ...
    it('imports ./home.module.css from home.tsx', async () => {
        await scaffoldLegacyProject(projectDir);
        const tsx = await fs.readFile(path.join(projectDir, 'home.tsx'), 'utf-8');
...
describe('refreshAgentMdIfNeeded', () => {
    ...
    it('is a no-op when agent.md already matches the current template', async () => {
        await fs.writeFile(path.join(projectDir, 'agent.md'), AGENT_MD_CONTENT, 'utf-8');
        const mtimeBefore = (await fs.stat(path.join(projectDir, 'agent.md'))).mtimeMs;
        // Small delay so mtime granularity (often 1ms or 1s on ext4) can
        // actually register a re-write if one happened.
        await new Promise((resolve) => setTimeout(resolve, 20));
        await refreshAgentMdIfNeeded(projectDir, 'nextjs');
        const mtimeAfter = (await fs.stat(path.join(projectDir, 'agent.md'))).mtimeMs;
    ...
    it('overwrites a stale CLAUDE.md with the current template', async () => {
        await fs.writeFile(path.join(projectDir, 'CLAUDE.md'), '# An old version of the loader\n', 'utf-8');
        await refreshAgentMdIfNeeded(projectDir, 'nextjs');
        const claudeMd = await fs.readFile(path.join(projectDir, 'CLAUDE.md'), 'utf-8');
        expect(claudeMd).toBe(CLAUDE_MD_CONTENT);
    });
