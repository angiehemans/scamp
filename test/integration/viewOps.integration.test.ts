...
describe('view operations', () => {
  ...
  const writeHomePage = async (): Promise<void> => {
  ...
  it('creates a view under views/ with the wrapper page at its slug', async () => {
    const created = await createComponent(
      { projectPath: dir, componentName: 'HeroCard', kind: 'view', wrapperSlug: 'hero-card' },
      'nextjs'
    );
  ...
  it('lists the view beside components and never lists its wrapper as a page', async () => {
    ...
    await createComponent(
      { projectPath: dir, componentName: 'Lobby', kind: 'view', wrapperSlug: 'lobby' },
      'nextjs'
    );
  ...
  it('leaves an existing page alone unless replacePage is set', async () => {
    ...
    await createComponent(
      { projectPath: dir, componentName: 'About', kind: 'view', wrapperSlug: 'about' },
      'nextjs'
    );
  ...
  it('converting the home page writes the wrapper at app/page.tsx', async () => {
    await createComponent(
      { projectPath: dir, componentName: 'Home', kind: 'view', wrapperSlug: 'home', replacePage: true },
      'nextjs'
    );
  ...
  it('refuses a view whose name a component already uses, and vice versa', async () => {
    ...
    await expect(
      createComponent({ projectPath: dir, componentName: 'Card', kind: 'view' }, 'nextjs')
    ).rejects.toThrow(/share one set of names/);
    await createComponent({ projectPath: dir, componentName: 'Lobby', kind: 'view' }, 'nextjs');
    await expect(
      createComponent({ projectPath: dir, componentName: 'Lobby' }, 'nextjs')
    ).rejects.toThrow(/share one set of names/);
  });
