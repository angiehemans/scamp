...
describe('textStyleLabel / textStyleTokenName', () => {
  ...
  it('builds a token name for a prop', () => {
    ...
    expect(textStyleTokenName('body-large', 'leading')).toBe(
      '--text-body-large-leading'
    );
  });
});
...
describe('buildTextStyles', () => {
  ...
  it('handles a multi-word style slug', () => {
    const styles = buildTextStyles(
      tokens({ '--text-body-large-size': '1.125rem' })
    );
...
describe('defaultTextStyleTokens', () => {
  ...
  it('round-trips into buildTextStyles with the PRD defaults', () => {
    ...
    expect(styles.map((s) => s.name)).toEqual(
      DEFAULT_TEXT_STYLE_TEMPLATES.map((t) => t.name)
    );
    ...
    expect(styles.find((s) => s.name === 'code')?.family).toBe(
      'var(--font-mono)'
    );
  });
});
