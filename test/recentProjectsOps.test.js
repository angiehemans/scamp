...
import { parseRecentStore, upsertRecent, setRecentFormat, removeRecentByPath, } from '../src/main/ipc/recentProjectsOps';
...
describe('removeRecentByPath', () => {
    it('drops the entry at the given path', () => {
        expect(removeRecentByPath([entry('/a'), entry('/b')], '/a').map((p) => p.path)).toEqual(['/b']);
    });
});
