...
import {
  ...
  parseBoxShadowSegment,
  parseBoxShadowShorthand,
  tokenizeShorthandSegment,
} from '@lib/parsers';
...
describe('tokenizeShorthandSegment', () => {
  ...
  it('keeps nested cubic-bezier intact', () => {
    expect(
      tokenizeShorthandSegment('opacity 200ms cubic-bezier(0.4, 0, 0.2, 1)')
    ).toEqual(['opacity', '200ms', 'cubic-bezier(0.4, 0, 0.2, 1)']);
  });
...
describe('parseBoxShadowShorthand', () => {
  ...
  it('parses a multi-shadow list and preserves order', () => {
    expect(
      parseBoxShadowShorthand(
        '0 4px 8px 0 rgba(0, 0, 0, 0.15), 0 1px 2px 0 rgba(0, 0, 0, 0.08)'
      )
    ).toEqual([
      {
        offsetX: 0,
  ...
  it('returns null when ANY segment fails (no silent drops)', () => {
    expect(
      parseBoxShadowShorthand('0 4px 8px 0 #000, var(--shadow-md)')
    ).toBeNull();
  });
  ...
  it('mixes inset and non-inset shadows in one list', () => {
    expect(
      parseBoxShadowShorthand('0 4px 8px #000, inset 0 0 0 1px #ffffff')
    ).toEqual([
      {
        offsetX: 0,
        offsetY: 4,
...
describe('formatBoxShadowShorthand', () => {
  ...
  it('joins multiple shadows with ", "', () => {
    expect(
      ...
      '0px 4px 8px rgba(0, 0, 0, 0.15), 0px 1px 2px rgba(0, 0, 0, 0.08)'
    );
  });
});

describe('parseBoxShadow round-trip', () => {
  const ROUND_TRIP_CASES: ReadonlyArray<BoxShadowDef> = [
    {
      offsetX: 0,
      offsetY: 4,
      blur: 8,
